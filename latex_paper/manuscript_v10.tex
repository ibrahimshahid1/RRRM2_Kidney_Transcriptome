...
\title{\textbf{Phenotype-anchored cross-cohort transcriptomics of the spaceflight kidney: RNA remodeling programs that co-vary with measured NCC/SPAK regulatory-phosphorylation suppression}\\[0.35em]
\large Anchoring RRRM-2, OSD-513, and OSD-253 transcriptomes to the measured distal-convoluted-tubule transport phenotype of OSD-462 (RR-10)}
...
\noindent\textbf{Purpose and scope of this draft.} This draft is framed as a \emph{phenotype-anchored cross-cohort transcriptomic study}. Its organizing principle is an evidence ladder that hangs the analysis off a \emph{measured molecular endpoint} --- suppression of NCC/SPAK regulatory phosphorylation in the distal convoluted tubule --- rather than off a moved pathway score. The definitive multi-omic, physiological, and morphological characterization of spaceflight-induced renal dysfunction, including this NCC dephosphorylation, has already been published as the Cosmic Kidney Disease study \citep{siew2024cosmic}; we do not re-discover it. Instead, the established phenotype is used as a fixed anchor, and the contribution is to ask whether transcriptomic remodeling programs --- built from datasets that were \emph{not} part of that study --- co-vary with that measured endpoint, both across cohorts and animal-by-animal within the anchor cohort, and to delineate honestly what transcriptomic pathway and network analysis can and cannot add.

\vspace{0.6em}
\small
\begin{tabularx}{\textwidth}{>{\bfseries}p{0.22\textwidth}X}
\toprule
Question & Using the established spaceflight DCT/NCC tubulopathy as a phenotype anchor, do transcriptomic remodeling programs co-vary with measured NCC/SPAK regulatory-phosphorylation suppression --- across independent mouse spaceflight RNA-seq cohorts, and animal-by-animal within OSD-462 (RR-10)? \\
\midrule
Datasets & RRRM-2 / OSD-771 is the factorial bridge cohort (age $\times$ arm $\times$ flight). OSD-513 is an independent kidney flight recurrence cohort. OSD-253 is a metadata-aware strain/mechanism cohort (C57BL/6J vs C3H/HeJ). OSD-462 / RR-10 is the multi-omic anchor cohort (RNA-seq, proteomics, phosphoproteomics); its kidney NCC dephosphorylation phenotype was first reported by \citet{siew2024cosmic}. Tabula Muris Senis kidney is an external aging reference. \\
\midrule
Relationship to prior work & The biological phenomenon --- spaceflight-associated renal transporter dephosphorylation and DCT remodeling --- is established by the Cosmic Kidney Disease study. This draft is confirmatory and integrative: it tests transcriptomic reproducibility across cohorts and maps the boundaries of network-based inference. It does not claim to discover the NCC phenotype. \\
\midrule
...
\midrule
Main supported result & A matrix-high / DCT-low remodeling direction recurs across RRRM-2 ISS-T, OSD-513, and OSD-462 RNA. In OSD-462, this RNA direction does \emph{not} translate to transporter or matrix protein abundance, but the WNK--SPAK/OSR1--NCC activating phosphorylation cluster is specifically and strongly suppressed. Kinase-substrate enrichment (KSEA) independently infers SPAK/OSR1 ($z=-6.31$) and WNK ($z=-4.12$) activity down --- a quantitative positive control on the Tier-1 phenotype. Animal-matched within RR-10 ($n=20$ flight + ground), the DCT transport RNA program and measured NCC regulatory phosphorylation are concordant at group level (RNA $-0.32$, phospho $-1.30$; regulatory cluster suppressed $\sim$4.3$\times$ more than non-regulatory sites); the per-animal link is positive but underpowered (Spearman $\approx +0.33$ to $+0.37$, $p\approx0.11$--$0.16$). \\
\midrule
Phenotype anchor & The paper hangs off a measured endpoint (NCC/SPAK regulatory phosphorylation) and an evidence ladder: RNA state recurrence $\rightarrow$ phospho activity (measured, Tier 1) $\rightarrow$ DCT morphology (Cosmic Kidney Disease, Tier 2 literature) $\rightarrow$ physiology (Tier 3, named as future work). \\
\midrule
Mechanism boundary & OSD-253 suggests TLR4/innate association, not TLR4 requirement. The RRRM-2 LAR arm shows attenuation, not a confirmed reversal; age-stratified effects are underpowered at $n=5$ per cell. The per-animal RNA$\leftrightarrow$activity link is suggestive, not significant; an honest control wrinkle (non-regulatory phosphosites also co-vary per-animal) is reported openly. \\
\bottomrule
\end{tabularx}
\normalsize
...
The matrix-high / DCT-low kidney flight direction is reproducible: it recurs across RRRM-2 ISS-T, OSD-513, and OSD-462 RNA. At the protein level in OSD-462 it is \emph{not} an abundance change; it corresponds instead to suppressed NCC/SPAK activating phosphorylation, concordant with the published Cosmic Kidney Disease phenotype \citep{siew2024cosmic}. This measured phosphorylation endpoint --- not a pathway score --- is the anchor: KSEA quantifies it as a kinase-activity loss (positive control), and an animal-matched comparison within RR-10 shows the DCT transport RNA program and NCC regulatory phosphorylation are concordant at group level, with a suggestive but underpowered per-animal link. The honest claim ceiling is group-level concordance plus a suggestive animal-level link, labelled as such. Transcriptomic network methods (LIONESS, node2vec) did not translate to protein-level support and are reported as a negative methodological result. RRRM-2 cannot support the original age-rewiring claim; the LAR arm shows attenuation rather than reversal.
...
\noindent
Spaceflight harms the kidney. A recent landmark study, Cosmic Kidney Disease, showed using kidney tissue, urine, and blood from mice, rats, and astronauts that spaceflight chemically switches off a key salt-handling transporter in the distal kidney tubule, called NCC. This work does not try to re-discover that. Instead it uses that established ``switch-off'' as a fixed, measured target --- a \emph{phenotype anchor} --- and asks: do gene-activity (RNA) patterns line up with it? We do this two ways. Across several \emph{separate} mouse kidney spaceflight experiments, the same RNA pattern recurs --- more matrix/scarring-type activity and less distal-tubule transport activity --- matching the NCC finding. Then, in the one cohort where the \emph{same kidneys} were measured for both RNA and protein modifications, we compare them animal-by-animal. The amount of the transporter protein barely changes, but its activating chemical modification is strongly reduced --- the same NCC switch-off, recovered here by a completely different route. A standard analysis method confirms the upstream control kinases (WNK and SPAK) are turned down, as expected. At the level of whole groups (flight versus ground), the RNA program and the measured switch-off move together. At the level of single animals the link is in the right direction but the experiment is too small to prove it --- and we say so plainly. We also tested whether fashionable ``network'' analysis methods added anything; honestly, they did not. The value of this paper is an honest, phenotype-anchored chain of evidence, not a new disease.

\vspace{0.8em}

\begin{abstract}
\noindent
The Cosmic Kidney Disease study established that spaceflight induces renal transporter dephosphorylation --- prominently of the thiazide-sensitive Na--Cl cotransporter NCC in the distal convoluted tubule (DCT) --- together with DCT remodeling, defining a primary spaceflight tubulopathy \citep{siew2024cosmic}. We do not re-investigate that phenotype; we use it as a \emph{phenotype anchor}. The central question is whether transcriptomic remodeling programs co-vary with that measured molecular endpoint --- suppression of NCC/SPAK regulatory phosphorylation --- both across independent mouse spaceflight cohorts and animal-by-animal within the anchor cohort, and we evaluate what transcriptomic pathway and network analysis contributes beyond standard differential expression. The analysis is organized as an evidence ladder: cross-cohort RNA-state recurrence, then measured NCC/SPAK phospho-activity as the Tier-1 anchor, then DCT morphology as a Tier-2 literature anchor, then physiology named as future work. We analyzed RRRM-2 (OSD-771), an age-by-arm-by-flight mouse kidney RNA-seq dataset, with the independent kidney flight cohorts OSD-513 and OSD-253, using residualized expression, curated pathway and mechanism scores, WGCNA modules, and LIONESS/node2vec network summaries combined into cross-cohort contrast vectors. We then built a pre-registered multi-omic anchor against OSD-462 (RR-10), which carries kidney RNA-seq, TMT proteomics, and phosphoproteomics on the \emph{same animals}; quantified the phenotype with kinase-substrate enrichment analysis (KSEA); and compared a per-animal DCT transport RNA score against a per-animal NCC regulatory-phosphorylation activity score in an animal-matched design.

RRRM-2 could not support the original within-cohort age-rewiring claim because control-aging directions were unstable at $n=5$ per cell. At the flight-vector level, the RRRM-2 ISS terminal-dissection arm aligned with OSD-513 (pathway cosine 0.641; 95\% CI 0.351 to 0.814) along a matrix-high / DCT-low tubulointerstitial direction; the live-return arm showed attenuation rather than a confirmed reversal. In the multi-omic anchor, the OSD-462 RNA flight effect reproduced the same matrix-high / DCT-low direction (pathway-vector cosine 0.87, 95\% CI 0.65--0.90; recurrence gate passed). Pre-registered protein-abundance concordance was \emph{null} for every targeted gene set, and matrix proteins moved opposite their transcripts; total NCC protein was unchanged ($+0.09$). However, the WNK--SPAK/OSR1--NCC activating phosphorylation cluster was specifically suppressed: NCC regulatory-cluster phosphosites (Thr53, Thr58, Thr65, Thr68; composite Ser65;Thr68 effect $-1.56$, $p=9.3\times10^{-7}$) and SPAK Ser383 ($-0.79$) were strongly down in flight, while non-regulatory NCC phosphosites were unchanged. KSEA, applied independently to the OSD-462 phosphoproteome, inferred SPAK/OSR1 activity down ($z=-6.31$, $p=2.8\times10^{-10}$) and WNK activity down ($z=-4.12$, $p=3.7\times10^{-5}$), a quantitative positive control confirming the Tier-1 phenotype. In the animal-matched comparison ($n=20$ flight and ground animals with RNA and phospho on the same kidneys), the DCT transport RNA program and NCC regulatory phosphorylation were concordant at group level (flight lowers both: RNA $-0.32$, phospho $-1.30$), and the regulatory phosphosite cluster was suppressed $\sim$4.3$\times$ more than non-regulatory NCC sites ($-1.30$ vs $-0.30$). The per-animal link was positive but underpowered (Spearman $\approx +0.33$ to $+0.37$, $p\approx0.11$--$0.16$; condition-adjusted likewise non-significant); it persisted when the \gene{Slc12a3} transcript was removed from the RNA score ($+0.30$). We report openly that non-regulatory NCC phosphosites also co-varied per-animal with the RNA score ($+0.55$, $p=0.011$), so the specificity evidence is the group-level magnitude difference, not the per-animal correlation. The honest claim ceiling is group-level concordance between the DCT transport RNA program and measured NCC regulatory phosphorylation, plus a suggestive but underpowered animal-level link. LIONESS/node2vec network candidates showed no enrichment for protein- or phosphoprotein-changing genes ($p\geq0.23$): a clean negative result on the added value of network methods. The contribution of this work is a phenotype-anchored, honestly labelled evidence ladder linking transcriptomic remodeling programs to a measured, clinically important renal spaceflight phenotype, an explicit transcript--protein--phosphoprotein dissociation, and a frank delineation of the limits of network-based inference.
\end{abstract}

\vspace{0.3em}
\noindent\textbf{Keywords:} spaceflight transcriptomics; mouse kidney; phenotype anchoring; evidence ladder; reproducibility; OSD-771; RRRM-2; OSD-513; OSD-462; RR-10; cosmic kidney disease; multi-omics; proteomics; phosphoproteomics; distal convoluted tubule; NCC; SLC12A3; WNK-SPAK/OSR1; kinase-substrate enrichment; KSEA; extracellular matrix; LIONESS; node2vec.
...
\subsection{The Cosmic Kidney Disease study defines the phenotype this work reproduces}
...
This manuscript is consequently framed as a reproducibility and methodological study rather than a discovery study. Independent confirmation matters in spaceflight biology specifically because flight cohorts are small, expensive, irreplaceable, and known to be sensitive to collection and preservation artifacts \citep{galazka2020rnaseq,preserve2016}, so any clinically relevant phenotype gains value from being shown to recur across cohorts and analytic routes. Three questions follow. First, does a renal remodeling \emph{direction} compatible with the Cosmic Kidney Disease phenotype reproduce across mouse spaceflight RNA-seq cohorts that were not part of that study? Second, when an orthogonal cross-cohort transcriptomic route is anchored against the RR-10 protein and phosphoprotein data, does it recover the established NCC signal --- and at which omic layer? Third, do widely used co-expression network methods add anything beyond standard pathway-level differential expression? The answers, developed below, are: yes; yes, at the phosphorylation/activity layer rather than the protein-abundance layer; and no.

\subsection{Phenotype anchoring and the evidence ladder}

A persistent weakness of high-dimensional transcriptomic analysis is that its primary objects --- pathway scores, contrast vectors, network metrics --- are statistical constructs whose biological meaning is asserted rather than measured. A pathway score can move without any measured physiological consequence, and a moved score is not, by itself, a phenotype. This manuscript is therefore organized around a deliberate methodological choice: rather than letting a moved pathway score be the endpoint, the analysis is hung off an externally established, \emph{measured} molecular endpoint and asks whether the transcriptomic constructs co-vary with it. We refer to this as phenotype anchoring, by analogy with the anchoring of molecular signatures to imaging or histological endpoints in other organ systems.
...
What this work adds is independent transcriptomic reproducibility and an explicit layered dissection. First, the matrix-high / DCT-low remodeling direction recurs across RRRM-2 (OSD-771) and OSD-513 --- cohorts that were not part of the Cosmic Kidney Disease study --- and then again in the OSD-462 RNA data, establishing that the direction is a reproducible cross-cohort transcriptomic feature rather than a single-study result. Second, anchoring the transcriptomic direction to the OSD-462 protein and phosphoprotein layers recovers the established NCC dephosphorylation signal by a route that begins from RNA and pathway contrasts, with the regulatory-versus-non-regulatory phosphosite contrast and the flat total-NCC-protein control providing internal evidence that the signal is genuine activating-site regulation. The convergence of an independent analytic route on a published result is a form of validation: it increases confidence in the underlying phenotype and demonstrates that the cross-cohort contrast-vector framework is sensitive to real renal-transport biology. The honest statement of novelty is that the contribution is reproducibility, integration, and methodological delineation, not a new disease mechanism.
...
This study is a phenotype-anchored cross-cohort transcriptomic analysis of the spaceflight kidney remodeling response. Rather than treating a moved pathway score as the endpoint, it hangs the analysis off a measured molecular phenotype --- suppression of NCC/SPAK regulatory phosphorylation --- and grades every transcriptomic claim against an explicit evidence ladder. A matrix-high / DCT-low remodeling direction recurs across three independent mouse kidney spaceflight cohorts (RRRM-2 ISS-T, OSD-513, and OSD-462 RNA), establishing the RNA-state rung as a reproducible feature rather than a single-study artifact. Anchored against OSD-462 (RR-10) protein and phosphoprotein data under a pre-registered plan, the transcriptomic direction does not correspond to transporter or matrix protein abundance but to specific suppression of the WNK--SPAK/OSR1--NCC activating phosphorylation cluster, independently reproducing the NCC dephosphorylation phenotype of the Cosmic Kidney Disease study \citep{siew2024cosmic}. KSEA quantifies that anchor as a kinase-activity loss and passes its pre-specified positive control, and an animal-matched comparison within RR-10 shows that the DCT transport RNA program and the measured NCC regulatory phosphorylation are concordant at group level, with the regulatory phosphosites suppressed about fourfold more than non-regulatory sites; the per-animal link is positive but underpowered, and is labelled as such. The contribution is therefore a phenotype-anchored, honestly graded evidence ladder, an explicit transcript--protein--phosphoprotein dissociation showing the signal is one of transport-pathway activity rather than abundance, and an honest negative result: co-expression network and embedding methods nominated candidates that did not translate to protein-level biology. RRRM-2 cannot support the original age-rewiring claim, and the live-return arm shows attenuation rather than reversal. The defensible value of the work is that it ties a transcriptomic remodeling program to a clinically relevant, measured renal spaceflight phenotype across cohorts and analytic routes, and delineates clearly --- rung by rung --- what transcriptomic and network analysis can and cannot establish on their own.
...
\end{center}
\vspace{0.3em}
\noindent\textit{Draft v10 prepared May 2026 --- the final draft. This draft is framed as a phenotype-anchored cross-cohort transcriptomic study; the spaceflight NCC dephosphorylation phenotype it anchors to was first established by the Cosmic Kidney Disease study \citep{siew2024cosmic}. Reference metadata should be curated against a journal style file before submission.}
