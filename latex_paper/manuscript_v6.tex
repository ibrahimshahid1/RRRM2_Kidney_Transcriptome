...
\title{\textbf{Cross-cohort contrast vectors identify an ISS-T-aligned matrix-high/DCT-low kidney remodeling state in mouse spaceflight transcriptomes}\\[0.35em]
\large RRRM-2 contrast analysis with OSD-513 recurrence, OSD-253 strain context, and live-return arm attenuation/divergence}
\author{Ibrahim Shahid}
\affil{Independent research draft for mentor feedback\\Authorship/collaboration structure pending}
\date{Draft v6, May 2026}
...
\midrule
Main supported result & RRRM-2 ISS-T aligns with an independent OSD-513 kidney flight vector, while RRRM-2 LAR does not show a detectable pooled matrix-minus-DCT flight response. The aligned ISS-T direction is a matrix-high / DCT-low tubulointerstitial state. LAR diverges from ISS-T at mechanism-score scale, but the apparent young-LAR opposite-direction state effect is underpowered and not statistically distinguishable from zero. Mechanism-axis projections nominate matrix/S1P/innate, clock-DCT, and tubular-transport programs as follow-up hypotheses rather than confirmatory subdivisions. \\
\midrule
...
\noindent
The original goal was to test whether spaceflight changes how mouse kidney aging networks behave. RRRM-2 is unusually useful for this because it includes young and old mice, flight and ground controls, and two mission arms. Each condition has only five mice, so the within-cohort aging-network direction is too noisy for the original broad age-rewiring claim. The supported result is arm-specific remodeling. The ISS terminal dissection arm of RRRM-2 points in the same pathway direction as an independent kidney spaceflight cohort, OSD-513. That shared direction looks like extracellular-matrix/fibrosis activation with distal tubule transport suppression. The live animal return arm does not show the same pooled state shift. Mechanism-score vectors suggest LAR divergence from ISS-T, and the young-LAR point estimate goes in the opposite state-score direction, but that age-stratified effect is not statistically resolved. OSD-253 helps constrain mechanism: TLR4/innate biology tracks the score, while the strain comparison argues for a broader remodeling program. LIONESS and node2vec prioritize candidate local network-neighborhood contributors to that program.

\vspace{0.8em}

\begin{abstract}
\noindent
Spaceflight affects renal physiology, but the extent to which mouse kidney transcriptomes show age-dependent network remodeling or recovery-associated redirection remains unclear. We analyzed RRRM-2 (OSD-771), an age-by-arm-by-flight mouse kidney RNA-seq dataset, together with external OSDR cohorts using residualized expression, curated pathway scores, WGCNA modules, LIONESS sample-specific network summaries, node2vec local-neighborhood displacement, cross-cohort contrast vectors, and a live-return arm attenuation/divergence analysis. RRRM-2 could not support the original broad age-rewiring claim because control-aging vectors were unstable at primary module/pathway resolution. The strongest result emerged at the arm-specific flight-vector level. RRRM-2 ISS-T aligned with OSD-513 at pathway scale (cosine 0.641; 95\% CI 0.351 to 0.814; directional permutation $p=0.127$), whereas RRRM-2 LAR anti-aligned directionally with OSD-513 (cosine $-0.511$; CI $-0.726$ to $-0.231$; directional permutation $p=0.178$).

The recurring ISS-T/OSD-513 direction was extracellular-matrix/fibrosis-positive and DCT/NCC-WNK transport-negative. A state-space decomposition represented this as a matrix-high / DCT-low tubulointerstitial state, with the DCT component kept in its native biological direction and the derived matrix-minus-DCT scalar kept separate from vector geometry. RRRM-2 ISS-T showed a strong positive state shift (matrix-minus-DCT effect $+2.254$, BH $q=0.020$), whereas LAR was near zero at the pooled level ($-0.112$, BH $q=0.895$). The pooled arm-by-flight interaction was significant ($2.366$, permutation $p=0.016$, BH $q=0.0267$), consistent with ISS-T response and LAR attenuation. Age-stratified effects were suggestive but underpowered: young ISS-T shifted strongly toward the state ($+3.204$, BH $q=0.025$), while young LAR had an opposite point estimate ($-0.782$) that was not statistically distinguishable from zero (Welch $p=0.361$, BH $q=0.542$). Mechanism-score vectors supported LAR divergence from ISS-T (cosine $-0.725$, bootstrap CI $-0.874$ to $-0.122$), and exploratory mechanism-axis projections nominate matrix/S1P/innate, clock-DCT, and tubular-transport programs for follow-up. OSD-253 constrained mechanism: the remodeling program was attenuated at the point-estimate level in C3H/HeJ but remained positive, and several ECM/fibrosis/adhesion/proteolysis scores remained positive in C3H/HeJ. WGCNA and LIONESS/node2vec nominated exploratory local-network contributors. These results support a reframed claim: RRRM-2 captures an ISS-T-aligned kidney remodeling state that recurs across cohorts, while live animal return primarily shows attenuation and mechanism-level divergence rather than a confirmed age-specific reversal.
\end{abstract}
...
\end{center}
\vspace{0.3em}
\noindent\textit{Draft v6 prepared May 2026. Reference metadata should still be curated against a journal style file before submission, but the current draft anchors the main biological and statistical claims to the relevant scholarly literature.}
