...
\title{\textbf{A recurrent transcriptomic context for distal tubule transporter suppression in mouse spaceflight kidney}\\[0.35em]
\large Mapping ECM, S1P, adhesion, proteolysis, innate, and stress-response programs around the established DCT/NCC spaceflight phenotype}
\author{Ibrahim Shahid}
\affil{Independent research draft for mentor feedback\\Authorship/collaboration structure pending}
\date{Draft v9, May 2026}
...
\noindent\textbf{Purpose and scope of this draft.} This draft is framed deliberately as a \emph{state-definition and context-mapping study}. It does not claim to discover that spaceflight affects the distal convoluted tubule (DCT), NCC, or the WNK--SPAK/OSR1--NCC phosphorylation axis. That biology was established by the Cosmic Kidney Disease study \citep{siew2024cosmic}. The contribution here is narrower but more biological than a methods paper: given the established DCT/NCC phenotype, we ask what transcriptomic programs consistently travel with a DCT/NCC-low kidney state across public mouse spaceflight datasets, how that state differs between terminal-flight and live-return/recovery contexts, and where RNA, protein abundance, phosphoprotein activity, and network-prioritized genes agree or diverge.

\vspace{0.6em}
\small
\begin{tabularx}{\textwidth}{>{\bfseries}p{0.22\textwidth}X}
\toprule
Question & What transcriptomic state surrounds the established spaceflight DCT/NCC transporter phenotype? Specifically, do DCT/NCC-low samples consistently co-express ECM/stromal, S1P, adhesion, proteolytic, innate, and stress-response programs, and does the state behave differently in terminal-flight versus live-return contexts? \\
\midrule
Datasets & RRRM-2 / OSD-771 is the primary context-mapping cohort (age $\times$ arm $\times$ flight). OSD-513 tests independent kidney RNA recurrence. OSD-253 tests strain/control-context sensitivity and TLR4/innate association. OSD-462 / RR-10 anchors the transcriptomic state to RNA, protein, and phosphoprotein layers; its kidney NCC dephosphorylation phenotype was first reported by Siew et al. \citep{siew2024cosmic}. Tabula Muris Senis kidney is an external aging reference. \\
\midrule
Relationship to prior work & The anchor phenotype --- spaceflight-associated DCT remodeling and NCC dephosphorylation --- is established. This draft uses that phenotype as a reference point and asks what RNA-state neighborhood surrounds it. It does not claim a new causal driver of NCC dephosphorylation. \\
\midrule
...
\midrule
Main supported result & A DCT/NCC-low terminal-flight RNA state recurs with ECM/stromal, S1P/S1PR3, integrin/adhesion, MMP/ADAM proteolysis, TLR/innate/macrophage, and preservation/stress-response programs. OSD-462 confirms that the transporter side of this state corresponds to the established NCC/SPAK phospho-suppression phenotype, while protein abundance and network-candidate translation remain null. \\
\midrule
Mechanism boundary & The state is an observational co-expression/context map, not causal proof. OSD-253 supports TLR4/innate association rather than TLR4 requirement. RRRM-2 LAR shows attenuation or redirection of the terminal-flight state, not a confirmed reversal. Age-stratified effects remain underpowered at $n=5$ per cell. \\
\bottomrule
\end{tabularx}
\normalsize
...
The paper defines a recurrent transcriptomic context for a known renal spaceflight phenotype. Terminal-flight kidney transcriptomes converge on a DCT/NCC-low state embedded in ECM/stromal, S1P, adhesion, proteolytic, innate, macrophage, and stress-response RNA programs. OSD-462 phosphoproteomics anchors the DCT/NCC side of that state to the published NCC/SPAK dephosphorylation phenotype, but the state does not become a broad matrix- or transporter-protein abundance claim. Network methods provide candidate local neighborhoods but do not validate at the protein/phosphoprotein layer. The novelty is the cross-dataset state map and its boundaries, not discovery of NCC dephosphorylation.
...
\noindent
Spaceflight kidney studies already show that the distal tubule and NCC transporter are affected. This paper asks a different question: when that known transporter phenotype appears in mouse kidney RNA data, what else appears with it? Across public mouse spaceflight kidney datasets, the low-DCT/NCC signal travels with a broader RNA state involving extracellular matrix, S1P signaling, adhesion, proteolysis, innate immune/macrophage context, and stress-response programs. That does not prove those pathways cause NCC suppression. It means they form the reproducible transcriptomic neighborhood around the phenotype. When the same state is checked against the RR-10 multi-omic dataset, the transporter side matches the known NCC/SPAK dephosphorylation finding, but protein abundance does not support simple transporter loss or matrix deposition. The value of this draft is therefore a biological context map: what surrounds the established DCT/NCC phenotype, which parts recur across cohorts, and which stronger mechanistic claims do not hold.

\vspace{0.8em}

\begin{abstract}
\noindent
The Cosmic Kidney Disease study established that spaceflight induces renal transporter dephosphorylation --- prominently of the thiazide-sensitive Na--Cl cotransporter NCC in the distal convoluted tubule (DCT) --- together with DCT remodeling, defining a primary spaceflight tubulopathy \citep{siew2024cosmic}. We treat that phenotype as an established anchor and ask what transcriptomic state surrounds it across public mouse kidney spaceflight datasets. We analyzed RRRM-2 (OSD-771), an age-by-arm-by-flight mouse kidney RNA-seq dataset, together with independent kidney cohorts OSD-513, OSD-253, and the RR-10 multi-omic cohort OSD-462. Using residualized expression, curated pathway and mechanism scores, WGCNA modules, LIONESS/node2vec network summaries, cross-cohort contrast vectors, and a pre-registered OSD-462 RNA/protein/phosphoprotein anchor, we mapped the RNA programs that co-vary with DCT/NCC suppression and tested which claims survive orthogonal omic checks.

RRRM-2 could not support the original within-cohort age-rewiring claim because control-aging directions were unstable at $n=5$ per cell. The strongest biological object was instead a terminal-flight DCT/NCC-low transcriptomic state. RRRM-2 ISS-T aligned with OSD-513 (pathway cosine 0.641; 95\% CI 0.351 to 0.814), and OSD-462 RNA reproduced the same direction (cosine 0.87, 95\% CI 0.65--0.90). This state coupled DCT/NCC-WNK suppression to ECM/stromal organization, integrin/adhesion, MMP/ADAM proteolysis, S1P/S1PR3 signaling, TLR/innate and macrophage context, and preservation/stress-response programs. The live-return arm showed attenuation or redirection rather than a confirmed reversal. OSD-253 supported TLR4/innate association but not TLR4 requirement. In OSD-462, pre-registered protein-abundance concordance was \emph{null} for every targeted gene set, and matrix proteins moved opposite their transcripts; total NCC protein was unchanged ($+0.09$). The transporter side of the state localized instead to the established phospho-activity layer: NCC regulatory phosphosites and SPAK sites were strongly suppressed while non-regulatory NCC sites were unchanged. LIONESS/node2vec candidates were not enriched for protein- or phosphoprotein-changing genes ($p\geq0.23$). We therefore define a recurrent transcriptomic context for the known spaceflight DCT/NCC phenotype, rather than a new causal axis: a terminal-flight RNA state in which DCT/NCC suppression travels with ECM, S1P, adhesion, proteolytic, innate, and stress-response programs, bounded by null protein-abundance and network-translation results.
\end{abstract}

\vspace{0.3em}
\noindent\textbf{Keywords:} spaceflight transcriptomics; mouse kidney; DCT/NCC state; distal convoluted tubule; NCC; SLC12A3; WNK-SPAK/OSR1; extracellular matrix; S1P; integrin; MMP/ADAM; innate immunity; OSD-771; RRRM-2; OSD-513; OSD-462; RR-10; cosmic kidney disease; multi-omics; proteomics; phosphoproteomics; LIONESS; node2vec.
...
\subsection{The Cosmic Kidney Disease study defines the anchor phenotype}

The scientific framing of this manuscript is constrained by an important piece of prior work. Siew et al. \citep{siew2024cosmic} reported an integrated pan-omic, physiological, and morphological study of spaceflight-induced renal dysfunction --- termed Cosmic Kidney Disease --- using kidney tissue, urine, and serum from mice, rats, and astronauts. Among its central findings, that study reported that spaceflight induces renal transporter dephosphorylation, that two of the most strongly dephosphorylated proteins were the thiazide-sensitive Na--Cl cotransporter NCC (encoded by \gene{Slc12a3}) and related distal nephron transporters, and that the distal convoluted tubule undergoes structural remodeling with increased segment size but reduced overall tubule density. Critically, the mouse kidney phosphoproteomics in that work derives from the Rodent Research-10 (RR-10) mission, which is the same experiment deposited in the NASA Open Science Data Repository as OSD-462 and used as the multi-omic anchor cohort in the present analysis. The Cosmic Kidney Disease study therefore already establishes the biological phenomenon --- a primary spaceflight tubulopathy with NCC dephosphorylation --- at a level of evidence (multi-species, multi-omic, physiological, morphological) that the present transcriptomic analysis does not attempt to surpass.

This manuscript is consequently framed as a context-mapping study rather than a discovery study of NCC biology. Independent confirmation still matters in spaceflight biology because flight cohorts are small, expensive, irreplaceable, and sensitive to collection and preservation artifacts \citep{galazka2020rnaseq,preserve2016}. But confirmation alone is not the full goal. The more useful question is what RNA-state neighborhood surrounds the established DCT/NCC phenotype: does DCT/NCC suppression appear alone, or does it consistently travel with stromal, immune, adhesion, proteolytic, S1P, and stress-response programs? Three questions follow. First, across independent mouse kidney flight datasets, what transcriptomic programs co-occur with a DCT/NCC-low state? Second, when that RNA state is anchored against RR-10 protein and phosphoprotein data, does it correspond to abundance-level remodeling, phosphorylation-level transporter regulation, or both? Third, do WGCNA, LIONESS, and node2vec nominate candidate gene neighborhoods that translate beyond RNA? The answers, developed below, are: a recurrent ECM/S1P/adhesion/proteolysis/innate/stress context; phosphorylation-level NCC/SPAK regulation without broad abundance concordance; and no independent protein/phosphoprotein support for the network candidate layer.
...
This is a property of the estimand. High-dimensional contrast-of-contrasts are harder than simple flight-versus-ground comparisons, especially for network features whose uncertainty is already larger than mean-expression uncertainty. The analysis below therefore separates the original age-rewiring hypothesis from the state-definition question that RRRM-2 can answer when paired with independent OSDR cohorts: what RNA programs define the terminal-flight DCT/NCC-low kidney state, and which parts persist, attenuate, or redirect after live return?
...
Sample-specific network methods such as LIONESS estimate how each sample contributes to a shared network structure \citep{kuijjer2019estimating}. Embedding methods such as node2vec provide low-dimensional summaries of graph neighborhoods \citep{grover2016node2vec}. These approaches are useful for asking whether a gene's local network context changes, in addition to whether its mean expression changes. Given the cohort size and multiple-testing burden, this manuscript uses LIONESS/node2vec as exploratory local-neighborhood prioritization after establishing the pathway/module-level state. The network layer is not allowed to become the claim unless it is supported by external or orthogonal evidence.

\subsection{A cross-cohort state-mapping strategy}

The strategy in this manuscript is to pair RRRM-2 with external OSDR cohorts at the level of comparable pathway and mechanism-state contrasts. RRRM-2 supplies the factorial bridge: it defines ISS-T and LAR flight directions under a controlled design and allows terminal-flight and live-return contexts to be compared. OSD-513 supplies independent kidney RNA recurrence. OSD-253 supplies strain and metadata-aware context because it includes C57BL/6J and C3H/HeJ mice and because C3H/HeJ has defective TLR4 signaling, making it useful for constraining TLR4-dependent interpretations. OSD-253 is analyzed as a mechanism-context cohort because original and rerun ground-control definitions trade off light history and sequencing/read-length comparability, and rodent spaceflight transcriptomes are known to be sensitive to sample collection and preservation factors \citep{galazka2020rnaseq,preserve2016}. Finally, OSD-462 (RR-10) supplies the multi-omic anchor: it is the only cohort in this analysis with matched kidney RNA-seq, TMT proteomics, and phosphoproteomics, and it is the cohort whose NCC phenotype was characterized by the Cosmic Kidney Disease study \citep{siew2024cosmic}. It therefore allows the transcriptomic state to be checked against protein abundance, phosphoprotein activity, and an externally established DCT/NCC benchmark.

The resulting analysis is organized as a deliberate claim hierarchy. First, RRRM-2 is tested for the original age-rewiring claim. Second, RRRM-2 arm-specific flight vectors are tested against OSD-513. Third, the aligned direction is decomposed into a DCT/NCC-low state and its co-traveling ECM, S1P, adhesion, proteolysis, innate, macrophage, and stress-response programs. Fourth, LAR is tested against three competing models: attenuation, true reversal, and mechanism switch. Fifth, OSD-253 is used to ask whether the state is TLR4-associated or TLR4-dependent. Sixth, WGCNA, LIONESS, and node2vec are used to annotate and prioritize genes within the pathway-level state. Seventh, the pre-registered OSD-462 multi-omic anchor tests whether the RNA state recurs and whether its orthogonal correlate is protein abundance, phospho-activity, or neither. Every step records which claims it supports and, equally, which it does not.
...
\subsection{A DCT/NCC-low transcriptomic state emerges across cohort context}
...
\textbf{Layer 4 --- RNA recurrence gate (PASS).} The OSD-462 RNA flight effect (Space~Flight minus Ground~Control) reproduced the RRRM-2 ISS-T DCT/NCC-low pathway direction with cosine $0.869$ (95\% bootstrap CI $0.648$ to $0.903$) across 11 pathways, with a leave-one-pathway-out cosine range of $0.823$ to $0.904$ (Figure~\ref{fig:osd462rna}). Because the direction did not depend on any single pathway, the recurrence gate passed and the protein/phospho layers were interpretable. This is the core state-map result: a third mouse kidney cohort, not used to define the original axis, recurs the same co-traveling RNA context.
...
Taken together, the anchor falsified protein-abundance concordance (H1, H2) and network translation (H4) and strongly supported phospho-axis suppression (H3). For v9, the interpretation is deliberately state-centered: the DCT/NCC-low RNA state recurs across cohorts and travels with ECM, S1P, adhesion, proteolysis, innate, macrophage, and stress-response programs, while its transporter-side orthogonal anchor is the already published NCC/SPAK phospho-suppression phenotype. The state should therefore be treated as transcriptomic context plus phospho-activity anchoring, not as evidence for transporter loss, matrix deposition, or a newly discovered NCC mechanism.
...
\begin{enumerate}
  ...
  \item The aligned ISS-T direction defines a terminal-flight DCT/NCC-low transcriptomic state embedded in ECM/stromal, S1P, integrin/adhesion, MMP/ADAM proteolysis, TLR/innate/macrophage, and preservation/stress-response programs.
  ...
  \item The state reproduces in a third independent cohort: the OSD-462 (RR-10) RNA flight effect recurs the RRRM-2 ISS-T direction (pathway-vector cosine $0.87$, 95\% CI $0.65$--$0.90$).
  \item In OSD-462, the RNA state does not translate to protein abundance --- targeted protein concordance is null, matrix proteins move opposite their transcripts, and total NCC protein is unchanged.
  \item The OSD-462 phosphoproteome anchors only the transporter-activity side of the state: the WNK--SPAK/OSR1--NCC activating phosphorylation cluster is specifically suppressed, with non-regulatory sites unchanged. This reproduces the Cosmic Kidney Disease NCC dephosphorylation phenotype \citep{siew2024cosmic} and is used as a positive-control anchor, not as a new discovery claim.
  \item WGCNA, LIONESS, and node2vec provide structural annotation and exploratory gene prioritization below the pathway/mechanism-state results; their candidate genes show no independent protein- or phosphoprotein-level support in OSD-462, which limits their role to candidate neighborhood mapping.
\end{enumerate}
...
The central biological result of this study is a state map, not a causal axis and not rediscovery of NCC biology. Across RRRM-2 ISS-T, OSD-513, and OSD-462 RNA, flight is associated with a coordinated DCT/NCC-low state in which extracellular-matrix organization, cell adhesion, proteolysis, innate/macrophage context, preservation/stress-response biology, and S1P/S1PR3-related features rise while distal convoluted tubule transport programs, especially DCT/NCC-WNK-linked biology, fall. This co-traveling transcriptomic neighborhood is the strongest coherent biological object recovered by the analysis.

This interpretation reframes the project away from a broad age-rewiring model and away from a method-validation-only paper. RRRM-2 was initially attractive because its factorial design could, in principle, ask whether spaceflight amplifies, dampens, or redirects age-associated kidney networks. In practice, the within-cohort control-aging directions were too unstable at primary module/pathway resolution to support that claim. The more defensible biology is a flight-associated transcriptomic context for the established DCT/NCC phenotype, with live return attenuating or redirecting the terminal-flight state.
...
What this work adds is not the DCT/NCC phenotype itself, but the transcriptomic context in which that phenotype appears across public mouse kidney spaceflight datasets. First, the DCT/NCC-low direction recurs across RRRM-2 (OSD-771), OSD-513, and OSD-462 RNA. Second, the co-traveling RNA programs are not arbitrary: they repeatedly involve ECM/stromal organization, adhesion, proteolysis, S1P/S1PR3, innate/macrophage context, and stress-response features. Third, anchoring the transcriptomic state to OSD-462 shows that the transporter side maps to the established NCC phospho-activity phenotype, while the abundance layer does not support transporter loss or broad matrix-protein accumulation. The honest novelty is therefore a cross-dataset RNA-state map around a known renal spaceflight phenotype, with explicit omic-layer boundaries.

\subsection{The transcript--protein--phosphoprotein dissociation}

A specific and useful result of the multi-omic anchor is a dissociation between omic layers. The transcriptomic remodeling direction reproduced cleanly at the RNA level, yet protein-abundance concordance was null for every targeted gene set, and matrix proteins moved opposite their transcripts. Total NCC protein abundance did not change. The signal that did move was phosphorylation: the SPAK/OSR1-target activating cluster on NCC was strongly and specifically suppressed. The biological reading is that the spaceflight kidney transcriptomic remodeling signature corresponds to a change in transport-pathway \emph{signaling activity} rather than to bulk changes in transporter or matrix protein abundance. This has a direct methodological consequence for spaceflight transcriptomics: an RNA-level ``DCT transport down'' signature should be interpreted as a candidate activity/regulation change to be confirmed at the phosphoprotein layer, not as evidence that transporter protein is lost. The matrix-protein result --- transcripts up, soluble proteins modestly down --- should be read cautiously, because bulk proteomics with sum-to-100 normalization can show compositional effects and because secreted matrix proteins may partition out of the soluble proteome; it is reported as an observation, not a mechanism.

\subsection{What network methods did and did not contribute}

A frank assessment of the network layer is warranted because network rewiring was the original motivation for the project. WGCNA provided interpretable module structure, and LIONESS/node2vec nominated candidate genes. But when those network-prioritized candidates were tested for independent support in the OSD-462 proteome and phosphoproteome, they showed no enrichment. The network layer therefore did not become an independent mechanism-discovery layer. Its useful role in v9 is narrower: it annotates candidate local neighborhoods inside the broader DCT/NCC-low state and identifies genes worth following up, while the central biological claim remains the cross-cohort pathway/mechanism-state map.

\subsection{A transcriptomic context for the DCT/NCC-low state}

The word context is used deliberately. The transcriptomic pattern is not simply generic differential expression, and it is not by itself proof of fibrosis, causality, or tissue deposition. It is a state signal in which matrix, adhesion, proteolysis, innate immune context, stress-response biology, S1P signaling, and tubular transport move together. In kidney biology, these processes are plausibly linked: renal injury and repair programs commonly involve epithelial stress, inflammatory signaling, fibroblast and matrix activation, altered cell-matrix adhesion, and changes in segment-specific transport identity \citep{liu2011fibrosis,natneph2024fibrosis,jci2024fibrosis,chavula2022cadherin}. The present data place the spaceflight-associated kidney response in that injury-repair/remodeling neighborhood without claiming that any one program is the upstream driver.
...
\subsection{Mechanistic neighborhood, not causal proof}

The mechanism-score results place the DCT/NCC-low state near ECM organization, adhesion/proteolysis, TLR4/innate signaling, macrophage/inflammatory context, S1P/S1PR3 biology, preservation/stress-response features, and DCT/NCC-WNK suppression. This is a plausible renal injury-remodeling neighborhood, but not a causal model. TLR4 and innate immune signaling can promote inflammatory and fibrotic kidney injury, TRIL is connected to TLR3/TLR4 signaling, integrins and cadherins participate in cell-matrix and fibroblast-like adhesion, MMP/ADAM proteases mediate matrix turnover, and S1P/S1PR3 signaling has recognized links to vascular, immune, inflammatory, and fibrotic responses \citep{souza2015tlr4,carpenter2009tril,carpenter2011tril,wochal2014tril,s1p2017,s1p2020,s1p2009}. These known relationships justify carrying the programs forward as candidate context, not as proven upstream causes of NCC dephosphorylation.
...
This study defines a recurrent transcriptomic context for the established spaceflight DCT/NCC phenotype. Across RRRM-2 ISS-T, OSD-513, and OSD-462 RNA, terminal-flight kidney samples converge on a DCT/NCC-low state embedded in ECM/stromal, S1P/S1PR3, integrin/adhesion, MMP/ADAM proteolysis, TLR/innate/macrophage, and preservation/stress-response programs. Anchored against OSD-462 (RR-10) protein and phosphoprotein data under a pre-registered plan, the state does not correspond to transporter or matrix protein abundance; instead, its transporter side maps to the already established WNK--SPAK/OSR1--NCC phospho-suppression phenotype reported by the Cosmic Kidney Disease study \citep{siew2024cosmic}. RRRM-2 cannot support the original age-rewiring claim, LAR supports attenuation or redirection rather than confirmed reversal, and LIONESS/node2vec network candidates do not translate to protein or phosphoprotein enrichment. The defensible value of the work is therefore biological state mapping with explicit boundaries: not discovery of NCC dephosphorylation, not proof of fibrosis, and not causal identification of upstream drivers, but a reproducible map of the RNA programs that travel with distal tubule transporter suppression in mouse spaceflight kidney.
...
\end{center}
\vspace{0.3em}
\noindent\textit{Draft v9 prepared May 2026. This draft is framed as a transcriptomic context map for the established spaceflight DCT/NCC phenotype; NCC dephosphorylation itself was first established by the Cosmic Kidney Disease study \citep{siew2024cosmic}. Reference metadata should be curated against a journal style file before submission.}
