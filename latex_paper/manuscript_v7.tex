...
\title{\textbf{Cross-cohort transcriptomics and OSD-462 multi-omics refine a matrix-high/DCT-low kidney remodeling state in mouse spaceflight}\\[0.35em]
\large RRRM-2 contrast analysis with RNA recurrence, protein-abundance falsification, NCC/SPAK phosphoproteomic support, and live-return arm attenuation/divergence}
\author{Ibrahim Shahid}
\affil{Independent research draft for mentor feedback\\Authorship/collaboration structure pending}
\date{Draft v7, May 2026}
...
\toprule
Question & Can RRRM-2 kidney RNA-seq support an age- and arm-dependent spaceflight network-remodeling claim, and does an independent kidney multi-omics cohort support the resulting DCT/NCC-WNK biology beyond RNA? \\
\midrule
Datasets & RRRM-2 / OSD-771 is the factorial bridge cohort. OSD-513 is the primary external RNA recurrence cohort. OSD-462 / RR-10 is the multi-omics anchor cohort with RNA-seq, TMT proteomics, and phosphoproteomics. OSD-253 is used as a metadata-aware mechanism/context cohort, especially for C57BL/6J versus C3H/HeJ and original versus white-LED rerun ground controls. Tabula Muris Senis kidney is used as an external aging reference. \\
\midrule
...
\midrule
Main supported result & RRRM-2 ISS-T aligns with independent OSD-513 and OSD-462 RNA flight vectors. The aligned RNA direction is a matrix-high / DCT-low tubulointerstitial state. OSD-462 proteomics does not support a simple matching protein-abundance version of that RNA state, but OSD-462 phosphoproteomics shows strong reduction of NCC regulatory phosphosites and SPAK sites, supporting post-translational suppression of NCC-pathway activity. RRRM-2 LAR does not show a detectable pooled matrix-minus-DCT flight response and is best treated as attenuation plus mechanism-level divergence. \\
\midrule
Mechanism boundary & OSD-253 suggests TLR4/innate association, not TLR4 requirement. OSD-462 sets a second boundary: transcriptomic matrix/DCT recurrence does not imply protein-abundance concordance, and LIONESS/node2vec network candidates do not enrich for protein- or phosphoprotein-changing genes. The strongest mechanistic support is instead phosphorylation-level NCC/SPAK suppression. \\
\midrule
...
RRRM-2 cannot support the original broad age-rewiring claim. RRRM-2 ISS-T aligns with OSD-513, and OSD-462 RNA independently recurs the same matrix-high and DCT/NCC-WNK-low direction. OSD-462 proteomics falsifies a simple protein-abundance translation of that RNA state, while phosphoproteomics supports reduced NCC/SPAK-axis phosphorylation. LAR is best described as attenuation at the pooled state-score level, with no detectable matrix-minus-DCT flight response; mechanism-score vectors suggest LAR divergence from ISS-T, and age-stratified patterns remain hypothesis-generating at $n=5$ per cell. OSD-253 suggests TLR4/innate association, not TLR4 requirement. WGCNA and sample-specific network methods are used as annotation and prioritization layers, not as validated protein-level mechanism-discovery tools.
...
\noindent
The original goal was to test whether spaceflight changes how mouse kidney aging networks behave. RRRM-2 is unusually useful for this because it includes young and old mice, flight and ground controls, and two mission arms. Each condition has only five mice, so the within-cohort aging-network direction is too noisy for the original broad age-rewiring claim. The supported result is arm-specific remodeling. The ISS terminal dissection arm of RRRM-2 points in the same pathway direction as independent kidney spaceflight RNA cohorts, including OSD-513 and OSD-462. That shared direction looks like extracellular-matrix/fibrosis activation with distal tubule transport suppression. OSD-462 then shows why this should not be over-read as a simple protein-abundance story: the targeted DCT, ECM, TLR4, and S1P protein sets do not concordantly mirror the RNA signal. Instead, the strongest mechanistic support appears in phosphoproteomics, where NCC regulatory phosphosites and SPAK sites are lower in flight. The live animal return arm does not show the same pooled state shift. OSD-253 helps constrain mechanism: TLR4/innate biology tracks the score, while the strain comparison argues for a broader remodeling program. LIONESS and node2vec prioritize candidate local network-neighborhood contributors, but those candidates do not independently translate into enriched OSD-462 protein or phosphoprotein changes.

\vspace{0.8em}

\begin{abstract}
\noindent
Spaceflight affects renal physiology, but the extent to which mouse kidney transcriptomes show age-dependent network remodeling, recovery-associated redirection, or multi-omic transporter support remains unclear. We analyzed RRRM-2 (OSD-771), an age-by-arm-by-flight mouse kidney RNA-seq dataset, together with external OSDR cohorts using residualized expression, curated pathway scores, WGCNA modules, LIONESS sample-specific network summaries, node2vec local-neighborhood displacement, cross-cohort contrast vectors, OSD-462 protein/phosphoprotein anchoring, and a live-return arm attenuation/divergence analysis. RRRM-2 could not support the original broad age-rewiring claim because control-aging vectors were unstable at primary module/pathway resolution. The strongest transcriptomic result emerged at the arm-specific flight-vector level. RRRM-2 ISS-T aligned with OSD-513 at pathway scale (cosine 0.641; 95\% CI 0.351 to 0.814; directional permutation $p=0.127$), whereas RRRM-2 LAR anti-aligned directionally with OSD-513 (cosine $-0.511$; CI $-0.726$ to $-0.231$; directional permutation $p=0.178$).

The recurring RNA direction was extracellular-matrix/fibrosis-positive and DCT/NCC-WNK transport-negative. A state-space decomposition represented this as a matrix-high / DCT-low tubulointerstitial state. OSD-462 RNA independently recurred this pathway direction (cosine 0.869; 95\% bootstrap CI 0.648 to 0.903), but OSD-462 protein abundance did not support a simple protein-level translation: DCT/NCC-WNK, ECM, TLR4, and S1P/S1PR3 protein-set effects were mostly null or opposite to the RNA-predicted direction. By contrast, OSD-462 phosphoproteomics supported reduced WNK-SPAK/OSR1-NCC activity, with Slc12a3/NCC regulatory phosphosites strongly decreased (regulatory-site mean $-0.832$ log$_2$ FL--GC) and SPAK/OSR1-axis sites decreased (mean $-0.405$). RRRM-2 ISS-T showed a strong positive matrix-minus-DCT state shift ($+2.254$, BH $q=0.020$), whereas LAR was near zero at the pooled level ($-0.112$, BH $q=0.895$), with a significant pooled arm-by-flight interaction ($2.366$, permutation $p=0.016$, BH $q=0.0267$). OSD-253 constrained mechanism by supporting TLR4/innate association rather than strict TLR4 requirement. LIONESS/node2vec candidates did not enrich among OSD-462 protein- or phosphoprotein-changing genes. These results support a reframed claim: mouse kidney spaceflight transcriptomes recover an ISS-T-aligned matrix-high/DCT-low RNA remodeling state, while independent multi-omics indicate that the most defensible DCT/NCC mechanism is phosphorylation-level suppression rather than protein-abundance loss or validated network-candidate translation.
\end{abstract}

\vspace{0.3em}
\noindent\textbf{Keywords:} spaceflight transcriptomics; mouse kidney; OSD-771; RRRM-2; OSD-513; OSD-462; OSD-253; proteomics; phosphoproteomics; LIONESS; node2vec; WGCNA; extracellular matrix; renal fibrosis; TLR4; S1P; distal convoluted tubule; NCC-WNK transport; SPAK/OSR1; live animal return.
...
The strategy in this manuscript is to pair RRRM-2 with external OSDR cohorts at the level of comparable pathway contrasts and, where available, independent molecular layers. RRRM-2 supplies the factorial bridge: it defines ISS-T and LAR flight directions under a controlled design. OSD-513 supplies an independent flight-vector recurrence test. OSD-462 / RR-10 supplies a multi-omics anchor because kidney RNA-seq, TMT proteomics, and phosphoproteomics were measured from the same spaceflight study, allowing the transcriptomic DCT/NCC-WNK hypothesis to be tested against protein abundance and phosphorylation. OSD-253 supplies mechanistic and metadata-aware context because it includes C57BL/6J and C3H/HeJ mice and because C3H/HeJ has defective TLR4 signaling, making it useful for constraining TLR4-dependent interpretations. OSD-253 is analyzed as a mechanism-context cohort because original and rerun ground-control definitions trade off light history and sequencing/read-length comparability, and rodent spaceflight transcriptomes are known to be sensitive to sample collection and preservation factors \citep{galazka2020rnaseq,preserve2016}.

The resulting hierarchy is deliberately simple. First, RRRM-2 is tested for the original age-rewiring claim. Second, RRRM-2 arm-specific flight vectors are tested against OSD-513 and the OSD-462 RNA layer. Third, the aligned direction is decomposed into a matrix-high / DCT-low tubulointerstitial state. Fourth, OSD-462 proteomics and phosphoproteomics ask whether the RNA state translates into protein abundance or post-translational transporter activity. Fifth, LAR is tested against three competing models: attenuation, true reversal, and mechanism switch. Sixth, OSD-253 is used to ask whether the state is TLR4-associated or TLR4-dependent. Seventh, WGCNA, LIONESS, and node2vec are used to annotate and prioritize genes within the pathway-level program, and OSD-462 is used to test whether those network-priority genes translate to protein/phosphoprotein evidence.
...
\subsubsection{OSD-462 / RR-10}

OSD-462 was used as the independent multi-omics anchor cohort. It contains RR-10 mouse kidney RNA-seq, TMT proteomics, and phosphoproteomics from basal, ground control, spaceflight, and vivarium-control contexts. The OSD-462 analysis did not pool samples with RRRM-2 or OSD-513. It asked whether the RRRM-2 ISS-T remodeling direction recurs at the OSD-462 RNA level and whether the DCT/NCC-WNK component is supported by OSD-462 protein abundance or phosphorylation. The primary RNA contrast was Space Flight minus Ground Control in the OSDR mRNA differential-expression table. The proteomics workbook sheet \code{protein\_quant\_2721} was used for protein abundance, and the phosphoproteomics sheets \code{siteQuant\_360} and \code{siteQuant\_360\_compositeSite} were used for phosphosite analysis.
...
\subsection{OSD-462 multi-omics anchor}

The OSD-462 analysis was designed as an anchor, not as a pooled replication. It used the same contrast logic as the cross-OSDR transcriptomic analysis: compare within-study Space Flight minus Ground Control effects and ask whether directions recur. OSD-462 RNA recurrence was tested by building a pathway-effect vector from the mRNA VST matrix and sample table, using the same mechanism gene sets mapped to Ensembl identifiers. Space Flight and Ground Control samples were resampled with replacement to generate a 2,000-replicate bootstrap confidence interval for the cosine between the OSD-462 RNA pathway vector and the fixed RRRM-2 ISS-T reference vector. Leave-one-pathway-out cosines assessed whether recurrence depended on a single pathway.

For proteomics, the processed TMT workbook was parsed from the \code{protein\_quant\_2721} sheet. Row-level metadata retained gene symbol, protein identifier, and peptide counts for both TMT plexes. Quantitative columns were identified from OSDR/Thermo sample labels and machine headers, restricted to scaled signal-to-noise channels. For each protein row, log$_2$ scaled values were median-centered per channel within plex, and the flight effect was estimated within each plex as
\begin{equation}
E_{g,p}^{protein}=\overline{\log_2 Y}_{g,p,FL}-\overline{\log_2 Y}_{g,p,GC},
\end{equation}
where $p$ indexes the two TMT plexes. The final protein flight effect was the mean of available plex effects, with the primary table requiring quantification in both plexes. Protein rows were collapsed to gene symbol by peptide-weighted averaging and then mapped to Ensembl identifiers using the OSD-462 RNA table as the preferred symbol bridge, with the project identifier map as fallback.

Protein concordance was tested for DCT/NCC-WNK transport, ECM organization, TLR4/innate signaling, S1P/S1PR3 signaling, and broad tubular transport as a control. For each set, three statistics were computed on protein-quantified members: signed mean protein effect, Spearman concordance between RRRM-2 ISS-T RNA effect and OSD-462 protein effect, and sign-agreement rate. The primary null was an abundance- and peptide-matched random-gene-set null. For each target set, 10,000 random gene sets of the same stratum composition were sampled from the protein-quantified background, matching jointly on protein abundance and peptide count. BH correction was applied across the four targeted families; broad tubular transport was reported as a control outside that BH family.

For phosphoproteomics, both \code{siteQuant\_360} and \code{siteQuant\_360\_compositeSite} were parsed. Per-site FL-minus-GC effects were estimated with a plex-adjusted linear model:
\begin{equation}
\log_2 Y_{ij} = \alpha_i+\beta_i I(\mathrm{Flight}_j)+\gamma_i I(\mathrm{Plex2}_j)+\epsilon_{ij},
\end{equation}
where $\beta_i$ is the phosphosite flight effect for site $i$. The hard activity checkpoint required quantified sites in \gene{Slc12a3}, \gene{Stk39}, and \gene{Oxsr1}. The NCC activity readout focused on Slc12a3 regulatory sites 53, 65, 68, 89 and observed composite regulatory sites including 53;65, 58;65, and 65;68. Reduced NCC-pathway activity was called only if the checkpoint passed, the WNK-SPAK/OSR1-NCC chain mean was negative, the NCC regulatory-site mean was negative, and at least one NCC regulatory site and one SPAK/OSR1 site were significantly down at nominal $p<0.05$. Protein-normalized phospho occupancy was also recorded as phosphosite effect minus matched protein-abundance effect where available.

Finally, OSD-462 was used to test whether the exploratory RRRM-2 LIONESS/node2vec candidate rankings translated into protein or phosphoprotein evidence. Top 25, 50, and 100 genes from the RRRM-2 composite ranking were tested against matched random nulls for mean absolute protein effect and per-gene maximum absolute phospho effect. This layer was interpreted as a falsification test of the network-prioritization layer, not as a primary discovery endpoint.
...
\subsection{OSD-462 recurs the RNA state but localizes DCT support to phosphorylation rather than protein abundance}

OSD-462 / RR-10 provided an independent multi-omics test of the transcriptomic state. At the RNA level, OSD-462 recurred the RRRM-2 ISS-T direction strongly. The OSD-462 RNA pathway vector aligned with the RRRM-2 ISS-T reference vector with cosine 0.869 across 11 shared mechanism pathways (2,000-bootstrap median 0.823; 95\% CI 0.648 to 0.903; Figure~\ref{fig:osd462recurrence}). Leave-one-pathway-out analysis remained positive throughout (minimum cosine 0.823), arguing that the RNA recurrence was not driven by a single pathway. ECM organization, fibrosis/TGF-beta/EMT, TLR4/innate signaling, integrin/cell adhesion, S1P/S1PR3, MMP/ADAM proteolysis, macrophage/inflammatory context, oxidative-stress/NRF2, and preservation-stress scores were positive in both vectors, whereas DCT/NCC-WNK transport was negative in both vectors. Broad tubular transport was the main sign-discordant pathway.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig_osd462_rna_recurrence.pdf}
\caption{\textbf{OSD-462 RNA recurrence and cross-layer DCT/NCC summary.} Panel A compares the OSD-462 RNA Space Flight-minus-Ground Control pathway vector with the RRRM-2 ISS-T RNA reference vector. Panel B shows the DCT/NCC axis across OSD-462 RNA, OSD-462 protein abundance, NCC regulatory phosphosites, and SPAK phosphosites. NCC RNA is strongly lower in flight, NCC protein abundance is slightly higher, and NCC/SPAK phosphorylation is lower.}
\label{fig:osd462recurrence}
\end{figure}

Protein abundance did not show the same targeted concordance. Among 6,418 genes with both RRRM-2 reference RNA effect and OSD-462 protein effect, the genome-wide RRRM-2 RNA-versus-OSD-462 protein correlation was near zero (Spearman $\rho=0.043$, Pearson $r=0.030$). Targeted gene-set tests also did not support a simple protein-abundance version of the RNA story (Table~\ref{tab:osd462protein}). The DCT/NCC-WNK set had a positive mean protein flight effect despite the predicted down direction (mean $+0.044$, matched-null $p=0.959$, BH $q=0.993$), and set-level cross-study RNA/protein concordance was near zero ($\rho=0.018$, $p=0.531$, BH $q=0.917$). The ECM organization set was opposite to the RNA-predicted up direction (mean protein effect $-0.107$, matched-null $p=0.993$), with negative cross-study concordance ($\rho=-0.396$, $p=0.917$). TLR4/innate and S1P/S1PR3 sets were also not supported at the protein-abundance layer. For the key DCT transporter \gene{Slc12a3}/NCC, OSD-462 RNA was strongly lower in flight (log$_2$ effect $-0.926$), but protein abundance was slightly higher ($+0.089$ across 70 peptides).
...
\toprule
Gene set & Direction predicted from RNA & Quantified genes & Mean protein effect & Signed-mean $p$ & Cross-study concordance & Concordance $p$ \\
\midrule
DCT/NCC-WNK transport & down & 10 & +0.044 & 0.959 & +0.018 & 0.531 \\
ECM organization & up & 15 & -0.107 & 0.993 & -0.396 & 0.917 \\
TLR4/innate & up & 8 & -0.021 & 0.797 & -0.405 & 0.881 \\
S1P/S1PR3 & up & 9 & -0.045 & 0.946 & +0.167 & 0.350 \\
Tubular transport broad & up & 15 & +0.043 & 0.069 & +0.071 & 0.398 \\
\bottomrule
\end{tabular}
}
\caption{\textbf{OSD-462 protein-abundance concordance tests.} Matched-null $p$ values compare each observed statistic against 10,000 abundance- and peptide-matched random gene sets. BH correction across the four targeted families yielded no supported protein-abundance concordance; broad tubular transport is shown as a specificity control.}
\label{tab:osd462protein}
\end{table}

The phosphoproteomics layer gave a different answer. The hard checkpoint passed: OSD-462 quantified \gene{Slc12a3}, \gene{Stk39}, and \gene{Oxsr1} phosphosites, with broad coverage of the WNK-SPAK/OSR1-NCC chain (16 quantified \gene{Slc12a3} sites, 5 \gene{Stk39} sites, 1 \gene{Oxsr1} site, 12 \gene{Wnk1} sites, and 8 \gene{Wnk4} sites). NCC regulatory sites were strongly lower in flight: \gene{Slc12a3} site 65 effect $-0.790$ ($p=3.46\times10^{-6}$), site 68 effect $-0.794$ ($p=3.32\times10^{-4}$), site 89 effect $-0.930$ ($p=6.64\times10^{-5}$), site 53 effect $-0.851$ ($p=3.77\times10^{-4}$), and the 65;68 composite site effect $-1.563$ ($p=9.30\times10^{-7}$). SPAK sites were also lower, including \gene{Stk39} site 366 ($-0.520$, $p=1.7\times10^{-5}$), site 383 ($-0.793$, $p=2.76\times10^{-4}$), and composite 382;383 ($-0.368$, $p=0.00187$). The NCC regulatory-site mean was $-0.832$, the SPAK/OSR1-axis mean was $-0.405$, and the chain mean across \gene{Slc12a3}, \gene{Stk39}, \gene{Oxsr1}, \gene{Wnk1}, and \gene{Wnk4} was $-0.211$.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig_osd462_multiomics_dashboard.pdf}
\caption{\textbf{OSD-462 multi-omics anchor dashboard.} Panel A shows RRRM-2 ISS-T RNA effect versus OSD-462 protein effect, highlighting targeted gene sets. Panel B shows DCT/NCC/WNK protein-abundance effects. Panel C shows WNK-SPAK/OSR1-NCC phosphosite effects with confidence intervals. Panel D tests whether RRRM-2 LIONESS/node2vec network-priority genes are enriched among OSD-462 protein- or phosphoprotein-changing genes.}
\label{fig:osd462dashboard}
\end{figure}

This separates abundance from activity. The OSD-462 data do not support a statement that NCC protein abundance is lower in flight. They do support a more specific statement: NCC regulatory phosphorylation and SPAK phosphorylation are lower in flight, consistent with reduced WNK-SPAK/OSR1-NCC pathway activity. This is also consistent with established distal-tubule biology, where NCC function is strongly regulated by phosphorylation rather than by abundance alone \citep{ge2016wnk,wnk2020,wnk2022}.

Finally, OSD-462 did not validate the exploratory LIONESS/node2vec candidate rankings as protein-level mechanism discovery. The top 25, top 50, and top 100 RRRM-2 composite-ranked genes were not enriched for larger absolute OSD-462 protein effects (top-25 $p=0.227$; top-50 $p=0.329$; top-100 $p=0.298$) or larger phosphoprotein effects (top-25 $p=0.592$; top-50 $p=0.254$; top-100 $p=0.751$). This negative result is important for the claim hierarchy: the network layer remains exploratory transcriptomic prioritization, while the stronger biological claim comes from recurrent RNA state plus phosphoproteomic NCC/SPAK support.
...
Node2vec and LIONESS add a gene-prioritization layer: they rank genes whose local transcriptomic network context moves with the ISS-T remodeling program. OSD-462 now places a stricter boundary around that layer. Because the top-ranked genes were not enriched for protein- or phosphoprotein-changing genes, these rankings should not be described as validated mechanism discovery. Their value is follow-up nomination and annotation inside the pathway/module result.
...
\begin{enumerate}
  ...
  \item OSD-462 RNA independently recurs the RRRM-2 ISS-T matrix-high / DCT-low direction.
  \item OSD-462 proteomics does not support a simple protein-abundance translation of the targeted DCT, ECM, TLR4, or S1P RNA story.
  \item OSD-462 phosphoproteomics supports reduced NCC/SPAK-axis phosphorylation, making post-translational transporter-activity suppression the strongest DCT/NCC mechanism supported outside RNA.
  ...
  \item WGCNA provides structural annotation. LIONESS and node2vec provide exploratory transcriptomic prioritization, but OSD-462 does not validate the network-priority genes as protein/phosphoprotein mechanism candidates.
\end{enumerate}
...
The central biological result of this study is not that different mission arms can differ. That is expected from the design. The result is the specific renal state that appears when kidney tissue is captured at terminal flight and the molecular layer that best supports its DCT/NCC component. Across RRRM-2 ISS-T, OSD-513, and OSD-462 RNA, flight is associated with a coordinated matrix-high / DCT-low tubulointerstitial state: extracellular-matrix organization, cell adhesion, proteolysis, innate/stress-associated biology, and S1P/S1PR3-related features rise, while distal convoluted tubule transport programs, especially DCT/NCC-WNK-linked biology, fall. OSD-462 then clarifies that this is not a simple protein-abundance story. Protein abundance only weakly mirrors the RNA state, whereas phosphoproteomics supports reduced NCC/SPAK-axis phosphorylation. This RNA-recurrent, phosphoprotein-supported DCT/NCC result is the strongest coherent biological object recovered by the analysis.

This interpretation reframes the project away from a broad age-rewiring model and away from a network-method-first story. RRRM-2 was initially attractive because its factorial design could, in principle, ask whether spaceflight amplifies, dampens, or redirects age-associated kidney networks. In practice, the within-cohort control-aging directions were too unstable at primary module/pathway resolution to support that claim. OSD-462 also shows that network-prioritized genes do not automatically become protein-validated mechanisms. The more defensible biology is a flight-associated tubulointerstitial remodeling program that recurs across independent kidney spaceflight RNA cohorts, is modulated by live return, and is mechanistically sharpened by NCC/SPAK phosphoproteomics.
...
\subsection{What the multi-omics anchor changes}

OSD-462 changes the interpretation in two ways. First, it strengthens the RNA recurrence claim. The OSD-462 RNA vector independently recurs the RRRM-2 ISS-T matrix-high / DCT-low direction, making the transcriptomic state less dependent on any single external cohort. Second, it prevents overclaiming. The targeted protein-abundance tests are mostly null or opposite: ECM RNA does not become a clean matrix-protein increase, and DCT/NCC-WNK RNA suppression does not become a lower NCC protein-abundance claim. This is not surprising in principle because transcript and protein abundances are non-redundant readouts influenced by translation, protein stability, detectability, timing, and assay sensitivity \citep{liu2020mrnaprotein}. In this dataset, the distinction is essential.

The strongest DCT/NCC support appears at the phosphorylation layer. That fits kidney physiology better than an abundance-only model. NCC activity in the distal convoluted tubule is controlled by the WNK-SPAK/OSR1-NCC phosphorylation cascade, and phosphorylation status can change transporter activity without requiring a matching change in total NCC abundance \citep{ge2016wnk,wnk2020,wnk2022}. Therefore, the OSD-462 result should be described as evidence for reduced NCC-pathway phosphorylation/activity, not as evidence for reduced NCC protein abundance. This interpretation also connects the present analysis to recent pan-omic spaceflight kidney work reporting renal transporter dephosphorylation and nephron remodeling as part of spaceflight-induced renal dysfunction \citep{siew2024cosmic}.

For the matrix side, OSD-462 imposes a parallel caution. Transcriptomic ECM activation may reflect transcriptional injury-repair signaling, altered cellular states, stress response, or early remodeling, but protein abundance and histology are needed before claiming extracellular matrix deposition. Thus, the manuscript should keep ``matrix-high'' as a transcriptomic state label and reserve ``fibrosis'' or ``matrix deposition'' for validated tissue or protein endpoints.
...
LIONESS and node2vec answer a different, secondary question: which genes may have local transcriptomic network neighborhoods that move with the ISS-T remodeling program? The top-ranked genes include stress-response, metabolic, clock-associated, transport, and signaling candidates, including \gene{Rbm3}, \gene{Per2}, \gene{Cyp4a14}, and \gene{Bhmt}. OSD-462 now gives this layer a direct falsification test, and the result is negative: top network-priority genes are not enriched among protein- or phosphoprotein-changing genes. These methods therefore do not establish a protein-validated hub-shift mechanism. Their value is to nominate transcriptomic candidate genes and local neighborhoods for follow-up inside the broader remodeling/transport state.

\subsection{Limitations}

Several limitations constrain the interpretation. First, RRRM-2 has only $n=5$ per Age $\times$ Arm $\times$ Environment cell, limiting high-dimensional interaction estimation and making age-stratified LAR patterns descriptive. Second, ISS-T and LAR differ in flight state, return/recovery timing, reloading, and collection context; their contrast should not be interpreted as a pure recovery experiment. Third, the clock-DCT result cannot be interpreted as circadian phase because RRRM-2 is not a circadian time-course experiment. Fourth, external OSDR cohorts differ in sex, strain, mission, duration, sequencing, and metadata. Cross-cohort agreement at pathway-vector level is therefore meaningful, but raw expression pooling would be inappropriate. Fifth, OSD-462 is a powerful molecular anchor but is still a separate mission and strain context; its proteomics are bulk whole-kidney TMT measurements, so DCT-specific abundance changes may be diluted. Sixth, phosphoproteomics supports reduced NCC/SPAK phosphorylation but does not itself measure electrolyte transport, urine chemistry, or stone formation. Seventh, OSD-253 has two imperfect control definitions, so it should remain a mechanism-context cohort rather than a clean validation endpoint. Eighth, curated mechanism signatures and bootstrap procedures cannot propagate every source of pipeline-level uncertainty.

\subsection{Future work}

The most direct follow-up is tissue-level and physiology-level validation of the remodeling/transport axis. Collagen staining, tubular injury markers, immune and macrophage markers, vascular/interstitial markers, and segment-specific NCC/WNK/SPAK assays would test whether the transcriptomic state corresponds to structural remodeling, tubular injury, or altered distal-nephron transport machinery. Because OSD-462 points to phosphorylation rather than abundance, phospho-NCC and phospho-SPAK/OSR1 immunostaining or targeted proteomics would be more directly relevant than total NCC alone. Urine and serum electrolytes, calcium handling, and stone-risk chemistry would be needed before linking the molecular signature to renal function or nephrolithiasis risk. The LAR-specific question is whether return and reloading preferentially attenuate matrix/S1P/innate activity while leaving distal transport programs incompletely restored. A design powered for Age $\times$ Arm $\times$ Flight interaction would be needed before age-specific recovery or reversal can be claimed.

Mechanistically, TLR4-intact and TLR4-impaired comparisons should be paired with S1P/S1PR3, TGF-beta/ECM, adhesion/proteolysis, macrophage/innate, and DCT transporter-phosphorylation markers. Computationally, the next useful step is not to add more network machinery, but to preserve the claim hierarchy: cross-cohort pathway/state recurrence first, multi-omic transporter-activity anchoring second, mechanism-context interpretation third, and network-neighborhood gene prioritization fourth.

% =====================================================================
\section{Conclusion}

RRRM-2's strongest estimable claim is a terminal-flight renal remodeling state that recurs across independent kidney RNA cohorts. The state is matrix-high and DCT/NCC-WNK-low, placing the kidney response near injury-repair, ECM/adhesion/proteolysis, innate/S1P-associated signaling, and distal transport suppression rather than simple accelerated aging. OSD-462 shows that this RNA state does not translate cleanly into targeted protein-abundance concordance, but it does support reduced NCC/SPAK-axis phosphorylation, making post-translational transporter regulation the most defensible DCT/NCC mechanism. Live animal return attenuates or redirects the matrix-associated component of this state more clearly than it restores distal transport biology. OSD-253 supports TLR4/innate association within a broader remodeling program, not strict TLR4 requirement. WGCNA provides structural annotation, while LIONESS and node2vec remain exploratory transcriptomic prioritization tools rather than protein-validated mechanism discovery.

\section*{Author contributions}
I.S. designed and implemented the analytical framework, conducted the contrast-vector, cross-OSDR, OSD-462 multi-omics, mechanism-axis, WGCNA-context, LIONESS/node2vec, and sensitivity analyses, generated the figures, drafted the manuscript, and maintained the source code, tests, and run artifacts. Authorship and collaboration structure will be revised with mentor and collaborator input.

\section*{Competing interests}
The author declares no competing interests.

\section*{Data and code availability}
NASA Open Science Data Repository accessions: OSD-771 (RRRM-2), OSD-513, OSD-462, and OSD-253. Tabula Muris Senis kidney was used as the external aging reference. Pipeline source, configuration files, run manifests, tests, figures, and manuscript drafts are version-controlled in the project repository.

\begin{itemize}
  \item Main v6 production run: \code{run\_20260519\_000547\_2500g}.
  ...
  \item Live-return attenuation/divergence outputs: \path{data/results/run_20260519_000547_2500g/contrast_vectors/mechanism_axis/tubulointerstitial_state/lar_reversal}.
  \item OSD-462 multi-omics anchor run: \code{run\_20260521\_osd462\_anchor\_v7}.
  \item OSD-462 anchor outputs: \path{data/results/run_20260521_osd462_anchor_v7/osd462_anchor}.
  \item 5,000-gene sensitivity run: \code{run\_20260519\_5000g\_sensitivity\_3seed}.
  \item Figure-generation scripts: \path{scripts/plot_manuscript_v4_figures.py}; \path{scripts/plot_lar_reversal_dashboard.py}; \path{scripts/osd462/05_plot_dashboard.py}.
  \item OSD-462 anchor scripts: \path{scripts/osd462/00_harmonize.py} through \path{scripts/osd462/05_plot_dashboard.py}.
...
\bibitem{liu2020mrnaprotein} Liu Y, Beyer A, Aebersold R. mRNAs, proteins and the emerging principles of gene expression control. \emph{Nature Reviews Genetics}. 2020;21:630--644. doi:10.1038/s41576-020-0258-4.
...
\bibitem{wnk2020} Uchida S, Shibata S. The WNK signaling pathway and salt-sensitive hypertension. \emph{Hypertension Research}. 2020;43:733--743. doi:10.1038/s41440-020-0437-x.

\bibitem{wnk2022} Gamba G. NCC regulation by WNK signal cascade. \emph{Frontiers in Physiology}. 2022;13:1081261. doi:10.3389/fphys.2022.1081261.
...
\end{center}
\vspace{0.3em}
\noindent\textit{Draft v7 prepared May 2026. Reference metadata should still be curated against a journal style file before submission, but the current draft anchors the main biological and statistical claims to the relevant scholarly literature and the OSD-462 multi-omics run artifacts.}
