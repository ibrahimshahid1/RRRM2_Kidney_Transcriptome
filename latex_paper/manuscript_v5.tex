...
\title{\textbf{Terminal-flight and live-return mouse kidney transcriptomes diverge along matrix/S1P, clock-DCT, and tubular-transport remodeling programs}\\[0.35em]
\large RRRM-2 contrast analysis with OSD-513 recurrence, OSD-253 strain context, and LAR reversal analysis}
\author{Ibrahim Shahid}
\affil{Independent research draft for mentor feedback\\Authorship/collaboration structure pending}
\date{Draft v5, May 2026}
...
\midrule
Main supported result & RRRM-2 ISS-T aligns with an independent OSD-513 kidney flight vector, while RRRM-2 LAR anti-aligns. The aligned ISS-T direction is a matrix-high / DCT-low tubulointerstitial state. A new LAR reversal analysis shows that young LAR moves in the opposite direction on the state score, and decomposes the arm divergence into three separable programs: matrix/S1P/innate activation in ISS-T and reversal in LAR; clock-DCT suppression in ISS-T with partial rebound in LAR; and persistent tubular-transport disruption in both arms. \\
\midrule
...
RRRM-2 cannot support the original broad age-rewiring claim. RRRM-2 ISS-T aligns with OSD-513, while LAR anti-aligns. The aligned ISS-T direction is matrix-high and DCT/NCC-WNK-low. LAR is not simply baseline recovery: in young mice it moves opposite to ISS-T on the matrix-minus-DCT state, with separable matrix/S1P reversal, clock-DCT rebound, and persistent tubular-transport disruption. OSD-253 suggests TLR4/innate association, not TLR4 requirement. WGCNA and sample-specific network methods are used as annotation and prioritization layers.
...
\noindent
The original goal was to test whether spaceflight changes how mouse kidney aging networks behave. RRRM-2 is unusually useful for this because it includes young and old mice, flight and ground controls, and two mission arms. Each condition has only five mice, so the within-cohort aging-network direction is too noisy for the original broad age-rewiring claim. The supported result is arm-specific remodeling. The ISS terminal dissection arm of RRRM-2 points in the same pathway direction as an independent kidney spaceflight cohort, OSD-513. That shared direction looks like extracellular-matrix/fibrosis activation with distal tubule transport suppression. The live animal return arm does not merely fade back to baseline. In young mice, LAR moves opposite to ISS-T on the matrix-minus-DCT state score. Mechanism decomposition separates this into matrix/S1P/innate reversal, partial clock-DCT rebound, and persistent tubular-transport disruption. OSD-253 helps constrain mechanism: TLR4/innate biology tracks the score, while the strain comparison argues for a broader remodeling program. LIONESS and node2vec prioritize candidate local network-neighborhood contributors to that program.

\vspace{0.8em}

\begin{abstract}
\noindent
Spaceflight affects renal physiology, but the extent to which mouse kidney transcriptomes show age-dependent network remodeling or recovery-associated redirection remains unclear. We analyzed RRRM-2 (OSD-771), an age-by-arm-by-flight mouse kidney RNA-seq dataset, together with external OSDR cohorts using residualized expression, curated pathway scores, WGCNA modules, LIONESS sample-specific network summaries, node2vec local-neighborhood displacement, cross-cohort contrast vectors, and a new LAR reversal/mechanism-switch analysis. RRRM-2 could not support the original broad age-rewiring claim because control-aging vectors were unstable at primary module/pathway resolution. The strongest result emerged at the arm-specific flight-vector level. RRRM-2 ISS-T aligned with OSD-513 at pathway scale (cosine 0.641; 95\% CI 0.351 to 0.814; directional permutation $p=0.127$), whereas RRRM-2 LAR anti-aligned with OSD-513 (cosine $-0.511$; CI $-0.726$ to $-0.231$; directional permutation $p=0.178$).

The recurring ISS-T/OSD-513 direction was extracellular-matrix/fibrosis-positive and DCT/NCC-WNK transport-negative. A state-space decomposition represented this as a matrix-high / DCT-low tubulointerstitial state, with the DCT component kept in its native biological direction. RRRM-2 ISS-T showed a strong positive state shift. LAR showed an age-specific opposite-direction pattern: young ISS-T shifted strongly toward the matrix-high/DCT-low state (matrix-minus-DCT effect $+3.204$), while young LAR shifted away from it ($-0.782$); old ISS-T shifted more weakly ($+1.304$), and old LAR did not reverse ($+0.558$). Mechanism-switch decomposition separated the LAR result into three programs: matrix/S1P/innate activation in ISS-T and reversal in LAR; clock-DCT suppression in ISS-T with partial rebound in LAR; and persistent tubular-transport disruption in both arms. OSD-253 constrained mechanism: the remodeling program was attenuated at the point-estimate level in C3H/HeJ but remained positive, and several ECM/fibrosis/adhesion/proteolysis scores remained positive in C3H/HeJ. WGCNA and LIONESS/node2vec nominated exploratory local-network contributors. These results support a reframed claim: RRRM-2 captures arm- and age-dependent divergence of a kidney remodeling state, with live animal return redirecting rather than simply erasing the terminal-flight response.
\end{abstract}

\vspace{0.3em}
\noindent\textbf{Keywords:} spaceflight transcriptomics; mouse kidney; OSD-771; RRRM-2; OSD-513; OSD-253; LIONESS; node2vec; WGCNA; extracellular matrix; renal fibrosis; TLR4; S1P; circadian clock genes; distal convoluted tubule; NCC-WNK transport; live animal return.
...
The resulting hierarchy is deliberately simple. First, RRRM-2 is tested for the original age-rewiring claim. Second, RRRM-2 arm-specific flight vectors are tested against OSD-513. Third, the aligned direction is decomposed into a matrix-high / DCT-low tubulointerstitial state. Fourth, LAR is tested against three competing models: attenuation, true reversal, and mechanism switch. Fifth, OSD-253 is used to ask whether the state is TLR4-associated or TLR4-dependent. Sixth, WGCNA, LIONESS, and node2vec are used to annotate and prioritize genes within the pathway-level program.
...
\end{equation}
Negative cosine indicates an opposite direction; negative $\beta$ indicates movement opposite the ISS-T direction with magnitude interpretable on the ISS-T scale. Bootstrap confidence intervals resampled samples within Arm $\times$ Age $\times$ condition cells. Permutation sensitivities shuffled FLT/GC labels within Arm $\times$ Age strata.
...
\end{equation}
Bootstrap intervals resampled within Arm $\times$ Age $\times$ condition cells, and permutation $p$ values shuffled FLT/GC labels within Arm $\times$ Age strata. BH correction was applied across the component-interaction family.

Mechanism-switch decomposition projected ISS-T and LAR effects onto hand-defined axes in mechanism-score space: matrix-high/DCT-low, clock-DCT state, S1P/matrix, preservation/cold-stress, immune/TLR4/macrophage, and transport rebound. The clock-DCT axis included core clock genes (\gene{Arntl}/\gene{Bmal1}, \gene{Clock}, \gene{Npas2}, \gene{Per1}, \gene{Per2}, \gene{Per3}, \gene{Cry1}, \gene{Cry2}, \gene{Nr1d1}, \gene{Nr1d2}, \gene{Rora}, \gene{Rorc}, \gene{Dbp}, \gene{Tef}, \gene{Hlf}) and DCT/WNK context genes. Without time-of-day metadata, this was interpreted as a clock-gene-associated transcriptional state, not as circadian phase. S1P analysis included sphingosine metabolism and receptor genes, with \gene{S1pr3} emphasized because it was a WGCNA hub. Rbm3 analysis evaluated \gene{Rbm3} as both candidate biology and preservation/cold-stress context marker.
...
\subsection{LAR redirects the ISS-T state rather than simply attenuating it}

The LAR arm was next tested against three competing models: attenuation/recovery toward baseline, true reversal of the ISS-T state, and mechanism switch into a different program. At the five-dimensional state-space level, the pooled LAR vector was opposite to ISS-T but weak in magnitude (cosine $=-0.493$, $\beta_{\mathrm{LAR}\rightarrow\mathrm{ISS}}=-0.124$), consistent with attenuation plus partial reversal. Age stratification made the result sharper. Young LAR was almost exactly opposite to young ISS-T in state-space direction (cosine $=-0.993$, directional permutation $p=0.0025$, BH $q=0.0375$), whereas old LAR was weak and inconclusive (cosine $=-0.133$).

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig12_lar_reversal_dashboard.pdf}
\caption{\textbf{LAR reversal and mechanism-switch dashboard.} Panel A shows RRRM-2 ISS-T and LAR gene-level FLT-minus-GC effects, with curated renal biology classes highlighted. Panel B summarizes cosine alignment across feature sets. Panel C shows arm-specific component effects. Panel D projects ISS-T and LAR effects onto mechanism-switch axes. Panel E shows targeted \gene{Rbm3}, \gene{Per2}, and \gene{S1pr3} expression effects. Panel F shows age-stratified matrix-minus-DCT state effects. The strongest single pattern is young ISS-T moving strongly toward the matrix-high/DCT-low state while young LAR moves in the opposite direction.}
\label{fig:larreversal}
\end{figure}

The age-specific matrix-minus-DCT effects provide the clearest biological summary. Young ISS-T moved strongly toward the matrix-high/DCT-low state (effect $=+3.204$), while young LAR moved in the opposite direction ($-0.782$). Old ISS-T showed a weaker positive shift ($+1.304$), and old LAR was also weakly positive ($+0.558$). The pooled arm-by-flight interaction for the matrix-minus-DCT state was positive and significant (interaction $=2.366$, bootstrap CI $0.817$ to $3.888$, permutation $p=0.0190$, BH $q=0.0317$). Matrix itself was the most robust component of this divergence: ISS-T increased and LAR decreased the matrix component (interaction $=1.738$, BH $q=0.0050$). DCT transport remained more complicated. The DCT component was lower in ISS-T and also lower in pooled LAR, and the pooled DCT interaction was not significant (BH $q=0.344$). Thus, the LAR signal is not simply ``DCT recovery.'' It is matrix/S1P/innate reversal with persistent tubular-transport disruption.

Mechanism-space analysis separated the LAR pattern into three programs. First, a matrix/S1P/innate program activated in ISS-T and reversed in LAR. The S1P/S1PR3 score increased in ISS-T and decreased in LAR (interaction $=0.997$, BH $q=0.0018$), and \gene{S1pr3} expression showed one of the strongest arm divergences (ISS-T effect $=+1.276$, LAR effect $=-1.269$, interaction $=2.545$, BH $q=0.0015$). ECM organization and TLR4/innate signaling showed the same ISS-T-positive/LAR-negative direction (BH $q=0.0018$ for both interactions).

Second, a clock-DCT transcriptional state was suppressed in ISS-T and partially rebounded in LAR. Core clock score decreased in ISS-T but was slightly positive in LAR (interaction $=-1.170$, BH $q=0.0015$); the Per/Cry subscore showed the same pattern (interaction $=-2.061$, BH $q=0.0030$). \gene{Per2} itself decreased strongly in ISS-T and was approximately unchanged in LAR (interaction $=-2.563$, BH $q=0.0024$). Across RRRM-2 samples, \gene{Per2} expression correlated positively with the DCT transport component ($\rho=0.449$, BH $q=7.5\times10^{-5}$) and negatively with matrix-minus-DCT ($\rho=-0.547$, BH $q=7.6\times10^{-7}$). Because RRRM-2 does not provide a time-series design or sufficient time-of-day metadata for phase inference, this result is described as a clock-gene-associated transcriptional state, not a measured circadian phase shift.

Third, tubular transport remained disrupted rather than cleanly restored. The transport-rebound mechanism axis was negative in both arms (ISS-T coefficient $=-0.35$, LAR coefficient $=-0.91$), and the DCT/NCC-WNK component did not show a significant arm interaction. This means the live-return arm does not look like a simple physiological normalization of distal-tubule transport. Instead, LAR reverses the matrix/S1P/innate part of the terminal-flight state, partially rebounds clock-associated DCT transcriptional markers, and retains or worsens transport disruption.

At broader feature levels, the reversal was clearest for mechanism scores and WGCNA module eigengenes rather than all genes. Pooled mechanism-score vectors showed LAR opposite to ISS-T (cosine $=-0.725$, 95\% bootstrap interval $-0.879$ to $-0.082$, directional permutation $p=0.0140$, BH $q=0.0750$). WGCNA eigengene vectors also pointed oppositely (cosine $=-0.489$, bootstrap interval $-0.764$ to $-0.059$), though permutation support was weaker. All-gene expression was much weaker (cosine $=-0.187$), and node2vec displacement magnitudes were almost perfectly positively aligned between arms, as expected for a non-directional movement metric. Therefore, the LAR result is a pathway/module and mechanism-level divergence, not a claim that the entire transcriptome or every local network metric reverses.
...
\begin{enumerate}
  ...
  \item RRRM-2 ISS-T aligns with an independent OSD-513 kidney flight direction, while RRRM-2 LAR anti-aligns.
  \item The aligned ISS-T direction is a matrix-high / DCT-low tubulointerstitial state.
  \item Young LAR moves opposite to young ISS-T on the matrix-minus-DCT state, whereas old LAR does not show the same opposite-direction pattern.
  \item Mechanism decomposition separates the arm divergence into matrix/S1P/innate activation in ISS-T and reversal in LAR, clock-DCT suppression in ISS-T with partial rebound in LAR, and persistent tubular-transport disruption in both arms.
...
The principal finding is arm-specific divergence of a kidney remodeling state in RRRM-2, anchored by external recurrence. RRRM-2 ISS-T aligns with OSD-513, and the aligned direction is biologically coherent: a matrix-high / DCT-low tubulointerstitial state with mechanism-score support for innate immune context, adhesion/proteolysis, S1P signaling, and macrophage/inflammatory programs. RRRM-2 LAR does not simply weaken this state. In young mice, LAR moves in the opposite matrix-minus-DCT direction, while old LAR shows a weaker and non-reversing pattern.

The analysis separates structural annotation from cross-cohort recurrence and mechanism interpretation. WGCNA identifies ECM/cell-migration module structure in ISS-T. Cross-OSDR contrast analysis shows that the ISS-T direction recurs in an independent kidney flight cohort. Mechanism scoring places the recurring direction in biologically plausible fibrosis/transport/innate-immune machinery. The LAR reversal analysis then shows that live animal return redirects selected parts of that program, especially matrix/S1P/innate biology, while tubular transport remains disrupted.

\subsection{Biological interpretation: terminal-flight and live-return divergence}
...
The new LAR analysis changes the biological interpretation. The live-return signal is not only absence of terminal-flight remodeling. It separates the state into three partially independent programs. The matrix/S1P/innate program increases in ISS-T and reverses in LAR, with \gene{S1pr3} showing especially strong arm divergence. The clock-DCT transcriptional state decreases in ISS-T and partially rebounds in LAR, with \gene{Per2} and Per/Cry markers tracking the DCT state across samples. The tubular transport program remains suppressed in both arms, especially in old LAR, which argues against simple recovery of distal-tubule transport after return.

This model is biologically more informative than a single-module interpretation. It suggests that terminal dissection captures an active matrix/innate/S1P remodeling state, whereas live return may capture resolution, redirection, reloading, circadian/clock-associated transcriptional rebound, or recovery-context biology superimposed on persistent transport disruption. The data do not distinguish these possibilities mechanistically, but they prioritize them for validation.
...
The LAR result adds two less-developed biological angles. First, S1P/S1PR3 signaling may connect interstitial, vascular, immune, and matrix remodeling responses, and it shows clean ISS-T-positive/LAR-negative behavior in this analysis. Second, clock-gene-associated distal-tubule biology is a plausible recovery-context axis because DCT transport regulation and systemic physiology are temporally regulated. Without time-of-day and phase information, however, the current data support only a clock-gene-associated transcriptional state, not circadian phase inference.

The important caveat is that transcriptomic pathway direction is molecular evidence for fibrotic and transport remodeling, not histology or physiology. It nominates a direction to test with collagen staining, tubular injury markers, immune staining, vascular/interstitial markers, urine/serum chemistry, segment-specific NCC/WNK markers, and perturbation studies.
...
The LIONESS/node2vec layer answers a secondary question: which genes may have local network neighborhoods that move with the recurrent remodeling program? The top-ranked genes include stress-response, metabolic, clock-associated, transport, and signaling candidates, including \gene{Rbm3} and \gene{Per2}. This layer can guide follow-up exploration, gene-set annotation, and figure supplements. It does not establish gene-level topology reversal. In fact, node2vec displacement magnitude is strongly shared between ISS-T and LAR because it measures movement magnitude rather than biological direction. The pathway/module and mechanism-state results remain the claim anchor; the network-neighborhood ranking supplies candidate genes for interpretation and validation.

\subsection{Limitations}

Several limitations are central. First, RRRM-2 has only $n=5$ per factorial cell, which limits high-dimensional interaction estimation and makes age-stratified LAR results descriptive even when the effect size is large. Second, ISS-T and LAR differ in flight state, return/recovery timing, reloading, and collection context; their contrast should not be interpreted as a pure recovery experiment. Third, the clock-DCT result cannot be interpreted as circadian phase because RRRM-2 is not a circadian time-course experiment and time-of-day metadata are insufficient for phase estimation. Fourth, external OSDR cohorts differ in sex, strain, mission, duration, sequencing, and metadata. Cross-cohort agreement at pathway-vector level is meaningful, while raw expression pooling would be inappropriate. Fifth, OSD-253 has two imperfect control definitions, and both must remain visible in the manuscript. Sixth, the mechanism signatures are curated and relatively small; coverage and de-overlap tables help audit them but cannot eliminate annotation bias. Seventh, the bootstrap resamples samples under fixed preprocessing, deconvolution, residualization, gene-set, and pathway-selection choices; it does not propagate all pipeline-level uncertainty. Eighth, Tabula Muris Senis provides an external aging reference with a larger age span than RRRM-2, so weak projection argues against simple aging mimicry but cannot rule out subtler age interactions.

\subsection{Future work}

The most direct experimental follow-up is to test the remodeling axis in tissue. Histology for collagen deposition, tubular injury, immune infiltration, vascular/interstitial state, and distal-nephron markers would distinguish transcriptomic remodeling from structural fibrosis. Segment-specific assays could test whether DCT/NCC-WNK suppression corresponds to altered NCC abundance or phosphorylation. The LAR-specific follow-up is to evaluate whether return/reloading redirects matrix/S1P/innate activity while leaving tubular transport impaired, and whether this effect is strongest in younger animals. Mechanistically, TLR4-impaired and TLR4-intact comparisons should be paired with S1P/S1PR3, TGF-beta/ECM, macrophage/innate, and DCT/WNK markers because OSD-253 points to a broader remodeling response. Computationally, the next step is to keep the claim hierarchy intact, expand external cohorts when metadata supports it, and use network-neighborhood rankings as hypothesis generation for targeted validation.

% =====================================================================
\section{Conclusion}

RRRM-2's strongest estimable claim is arm-specific divergence of a kidney remodeling state. The ISS-T direction recurs in OSD-513 and is best described as a matrix-high / DCT-low tubulointerstitial state embedded in TLR4/innate, adhesion/proteolysis, S1P, macrophage/inflammation, and sample-context biology. LAR does not simply erase this state: in young mice it moves opposite to ISS-T on the matrix-minus-DCT score, with mechanism decomposition separating matrix/S1P/innate reversal, clock-DCT rebound, and persistent tubular-transport disruption. OSD-253 places TLR4/innate signaling as an associated context within a broader remodeling program. WGCNA and LIONESS/node2vec provide annotation and exploratory prioritization layers below the cross-cohort pathway/module and mechanism-state claim.
...
\begin{itemize}
  \item Main v5 production run: \code{run\_20260519\_000547\_2500g}.
  ...
  \item LAR reversal and mechanism-switch outputs: \path{data/results/run_20260519_000547_2500g/contrast_vectors/mechanism_axis/tubulointerstitial_state/lar_reversal}.
  \item 5,000-gene sensitivity run: \code{run\_20260519\_5000g\_sensitivity\_3seed}.
  \item Figure-generation scripts: \path{scripts/plot_manuscript_v4_figures.py}; \path{scripts/plot_lar_reversal_dashboard.py}.
...
\end{center}
\vspace{0.3em}
\noindent\textit{Draft v5 prepared May 2026. Reference metadata should still be curated against a journal style file before submission, but the current draft anchors the main biological and statistical claims to the relevant scholarly literature.}
