...
\title{\textbf{Spaceflight kidney remodeling RNA is decoupled from protein abundance, while NCC/WNK transporter suppression resolves at phosphorylation}\\[0.35em]
\large Cross-cohort RNA recurrence and matched OSD-462 multi-omics of the mouse distal nephron}
...
\begin{abstract}
\noindent
Spaceflight injures the kidney, and the sodium-chloride cotransporter NCC and its WNK--SPAK/OSR1 regulatory kinases are dephosphorylated in flight; whether the accompanying transcriptomic response reflects that endpoint across protein and phosphorylation layers was unknown. We provide the first matched cross-layer decomposition of the spaceflight kidney response, using public mouse RNA-seq, TMT protein, and phosphoproteomic datasets (RRRM-2/OSD-771, OSD-513, OSD-253, and matched RR-10/OSD-462) with external DCT1/DCT2 single-nucleus, perturbation, spatial, and human-urine references. The bulk RNA response indexes a remodeling-rich tissue context that does not reach protein abundance, leaving NCC/SPAK/WNK regulatory phosphorylation as the layer that carries the transporter lesion.

Across independent RNA cohorts, flight raised matrix/endothelial remodeling and lowered DCT/NCC-WNK transport scores; in a five-cohort meta-analysis remodeling recurred reliably ($p=7.0\times10^{-4}$) while transport suppression was directional but variable across cohorts. In matched OSD-462 multi-omics this RNA pattern did not propagate to protein: RNA and protein flight effects were decoupled (genome-wide Spearman $-0.034$), no curated set propagated cleanly from RNA to protein abundance, and the one set with a null-tested protein effect---matrix/ECM---was actively inverted, so a reader with RNA alone would wrongly infer protein-level remodeling. DCT/NCC-WNK RNA fell while total NCC and related protein abundance stayed flat-to-slightly-positive; the transporter-regulatory signal instead appeared at NCC/SPAK/WNK phosphosites, where an unbiased kinome-wide screen independently recovered WNK1/WNK3 suppression.

Mapping flight-suppressed phosphosites to parent genes and ranking them against a native DCT1/DCT2 prior, we found enrichment at distal-nephron subtype-prior extremes, led by a DCT2-leaning bin that survived an observability-matched parent-gene permutation; the DCT1-high bin was strongly enriched at the phosphosite-row level but did not survive it. Public perturbation and spatial references retained this enrichment under parent-protein normalization and nominated DCT-adjacent remodeling neighborhoods as a spatial hypothesis.

The spaceflight kidney transcriptome is therefore a tissue-context readout that does not propagate to protein, and DCT-subtype-resolved or spatial phosphoproteomics, rather than additional bulk transcriptomics alone, is the decisive next experiment.
\end{abstract}
...
Kidney risk has long been part of human spaceflight medicine, initially framed around altered calcium balance and renal stone risk \citep{cockett1962astronautical,pietrzyk2007renal,smith2014men}. In microgravity the loss of a gravitational gradient drives a cephalad fluid shift---a redistribution of roughly two litres of blood and interstitial fluid toward the thorax and head---that alters central volume sensing and the renal handling of sodium and water, making the kidney a direct target of spaceflight physiology \citep{spacekidney2023,christensen2001renal,drummer2001water}. More recent work has shifted the emphasis from stone physiology to active tubular dysfunction, describing renal tubulopathy, interstitial and vascular remodeling, mitochondrial and stress responses, and altered solute handling \citep{afshinnekoo2020fundamental,daSilveira2020comprehensive,chouker2025impact}, much of it enabled by rodent studies in which kidney tissue is profiled directly across transcriptomic, proteomic, phosphoproteomic, and histologic layers.

The Cosmic Kidney Disease study reported integrated pan-omic, physiological, and morphological evidence of spaceflight-induced renal dysfunction across mice, rats, and astronauts, including dephosphorylation of the sodium-chloride cotransporter NCC (\gene{Slc12a3}) and related distal-nephron transport machinery together with distal convoluted tubule (DCT) remodeling \citep{siew2024cosmic}. The mouse kidney phosphoproteomics in that work derives from Rodent Research-10, deposited as OSD-462, which includes kidney RNA-seq, TMT proteomics, and phosphoproteomics for flight and ground-control mice. We analyze OSD-462 alongside independent mouse kidney RNA-seq cohorts to define the flight RNA signature, compare RNA and protein abundance across remodeling and distal-nephron programs, quantify NCC/SPAK/WNK regulatory phosphosite changes, and rank suppressed phosphosite parent genes against an external DCT1/DCT2 reference.
...
\end{tikzpicture}
\caption{\textbf{The WNK--SPAK/OSR1--NCC regulatory axis and its activation state.} WNK kinases activate SPAK and OSR1, which phosphorylate the N-terminal regulatory cluster of NCC; phosphorylated NCC is active and drives distal sodium-chloride reabsorption (top), whereas dephosphorylated NCC is inactive (bottom). In OSD-462 spaceflight kidney this regulatory phosphorylation is suppressed while total NCC protein abundance is unchanged, shifting NCC toward the inactive state---so the transporter lesion is visible at the phosphorylation layer, not at protein abundance.}
\label{fig:axis}
\end{figure}

Bulk kidney RNA-seq provides a broad tissue-context readout but cannot by itself distinguish DCT transcriptional suppression, DCT segment loss or dedifferentiation, and dilution by expanding interstitial or endothelial compartments. A recurring RNA pattern with increased matrix/endothelial remodeling and decreased DCT/NCC-WNK transport could also fail to produce concordant protein-abundance changes. For this reason, the analysis combines cross-cohort RNA recurrence with OSD-462 protein and phosphoprotein contrasts, then uses an external native DCT1/DCT2 single-nucleus reference to prioritize parent genes of suppressed phosphosites.

We tested three related hypotheses. First, public mouse kidney spaceflight RNA cohorts would show a reproducible tissue-context pattern characterized by increased matrix/endothelial remodeling and lower DCT/NCC-WNK transport scores. Second, in the OSD-462 flight-versus-ground multi-omic subset, this RNA pattern would not be recapitulated at protein abundance and would instead be most evident at NCC/SPAK/WNK regulatory phosphorylation. Third, flight-suppressed whole-kidney phosphosites would be enriched among parent genes with distal-nephron subtype-prior expression, identifying candidate DCT1- and DCT2/CNT-associated parent-gene sets for follow-up. To address these, we combined within-study gene-set scoring and cross-cohort pathway-vector recurrence in the RNA cohorts, OSD-462 TMT protein and phosphosite contrasts with targeted and kinome-wide KSEA, and native DCT1/DCT2 parent-gene enrichment, with composition-aware, perturbation-reference, spatial-reference, and human fluid-axis analyses as sensitivity and context layers.

The resulting model places the recurring RNA response in a matrix/endothelial-high tissue context with directional DCT/NCC-WNK transport suppression, while OSD-462/RR-10 data place the transporter-regulatory signal primarily at the phosphorylation layer. The DCT-prior enrichment is parent-gene based and whole-kidney, so its experimental implication is specific: spatial, DCT-enriched, and physiology-coupled studies should test whether the suppressed transporter-regulatory program is DCT-intrinsic, associated with DCT-adjacent remodeling neighborhoods, or distributed across a broader distal-nephron response. The fork is consequential: a DCT-intrinsic lesion points to direct transporter regulation as the countermeasure target, whereas a remodeling-associated one implies that transporter suppression is secondary to interstitial/endothelial expansion.
...
Across all tested RNA cohorts, flight raised matrix/endothelial remodeling scores and lowered DCT/NCC-WNK transport scores, and this paired response recurred between independent cohorts. RRRM-2 ISS terminal flight effects aligned with OSD-513 at pathway level (cosine 0.641, bootstrap 95\% CI 0.373 to 0.811). The live-return arm showed attenuation and directional anti-alignment, consistent with recovery context. The planned age-by-flight network question is reported in the compendium because its module/pathway aging directions did not hold a consistent sign under bootstrap resampling.

The recurring RNA pattern combined increased matrix, endothelial/stromal, adhesion, proteolysis, innate/inflammatory, and stress-associated features with decreased DCT/NCC-WNK transport. OSD-462 RNA independently reproduced the RRRM-2 ISS terminal direction with a pathway-vector cosine of 0.869 (95\% CI 0.65--0.90). This whole-panel agreement is carried by the remodeling pathways; the transport component is isolated separately in the five-cohort meta-analysis below, where it is the weaker half.

A gene-set-level recurrence meta-analysis across five OSDR cohorts (OSD-102, OSD-163, OSD-253, OSD-462, OSD-513) distinguished the two halves of the signature; RRRM-2 ISS terminal served as the fixed reference vector for the pairwise cosine recurrence above rather than as a member of this meta. The matrix/ECM-high set recurred by weighted Stouffer combination \citep{whitlock2005zmethod} ($p=7.0\times10^{-4}$) and remained significant under leave-one-cohort-out, whereas the DCT/NCC-WNK transport set was directionally lower, weaker, and not sign-stable across cohorts (Stouffer $p=0.19$, median FDR 0.95, median $I^2=73\%$, substantial between-cohort heterogeneity \citep{higgins2002heterogeneity}). Because OSD-462 is one of the five cohorts, the remodeling recurrence is not fully independent of OSD-462; it persists when that cohort is dropped. The recurrent RNA component was strongest for remodeling, and the transport decrease was a weaker, directional RNA feature.
...
At genome scale, RNA and protein flight effects were decoupled rather than inverted (Spearman $-0.034$): the two layers carry largely independent information. Across the curated panel, no set propagated cleanly from RNA to protein abundance, and the one set with a protein effect testable against a matched null---matrix/ECM (above)---was actively inverted, while the remaining discordant sets had small protein effects whose direction we report descriptively (Table~\ref{tab:crosslayer}, Fig.~\ref{fig:crosslayer}). The interpretive consequence is direct: a reader with only the RNA data would infer that matrix-remodeling proteins accumulate in the spaceflight kidney, which the protein layer does not show. The recurrent RNA signature is therefore a tissue-context readout shaped by which cell types are present, as expected when expanding interstitial and endothelial compartments dilute per-cell signals and composition shifts drive apparent expression changes independently of per-cell regulation \citep{nido2020composition,avilacobos2020deconv}---not a protein-output program. Read at face value the RNA layer is actively misleading about protein remodeling; only the matched protein and phosphorylation layers recover the transporter-regulatory lesion.
...
A continuous, matched-null-calibrated version of this per-pathway pattern sharpens the descriptive direction calls into explicit layer assignments (Methods; \path{src/v11/rna_protein_propagation.py}). Two pathways emerge as calibrated protein inversion, with the protein layer moving opposite the RNA pathway direction beyond the abundance- and peptide-count-matched null: matrix/ECM (signed-mean protein effect $-0.107$, inverse-signed-mean BH $q=0.091$) and TLR4/innate (inverse-slope BH $q=0.078$). Three pathways read instead as RNA-to-phospho-carry candidates, where the RNA pathway direction is carried at the phosphosite parent mean rather than at protein abundance: DCT/NCC-WNK transport (31\% of RNA-directional members carried at phospho, $n=13$), broad tubular transport, and S1P/S1PR3. The remaining six pathways are not propagated within the matched null (Fig.~\ref{fig:propagation}), with Benjamini-Hochberg correction applied within the 11-pathway family. The matrix/ECM inversion and the DCT/NCC-WNK phospho-carry were the two pre-registered layer-assignment outcomes, and both survive this calibration.
...
The transporter lesion instead appeared at the regulatory-phosphorylation layer. Because NCC is activated by WNK--SPAK/OSR1 phosphorylation of its N-terminal regulatory sites, this layer rather than total abundance reports transporter activation, so flat NCC protein with suppressed regulatory phosphosites is consistent with reduced activation despite preserved abundance. NCC N-terminal phosphosite features fell by approximately 0.79--0.99 log$_2$ units, and the composite p65;p68 feature fell by $-1.563$ ($p=9.3\times10^{-7}$). SPAK position features p366 and p383 were also suppressed. Non-regulatory NCC sites were comparatively flat. These suppressed N-terminal and SPAK sites are concordant with the antibody-validated NCC dephosphorylation reported by Cosmic Kidney Disease \citep{siew2024cosmic}. In OSD-462, the NCC/SPAK/WNK phosphorylation pattern occurs without a corresponding decrease in total NCC protein abundance or broad RNA-to-protein propagation of the flight RNA program. Targeted KSEA over three curated substrates per kinase was directionally consistent with reduced SPAK/OSR1 and WNK axis output; the $z$ magnitudes index substrate concordance over three partially dependent sites, not a powered effect size, and are read as pathway coherence rather than independent kinase discovery.

The unbiased kinome-wide motif screen against the Johnson et al.\ Ser/Thr kinase atlas \citep{johnson2023atlas} independently corroborated the WNK arm without using that curated substrate set: \gene{WNK1} was among the most suppressed kinase outputs genome-wide ($z=-4.87$, $q=5.4\times10^{-6}$, 101 atlas substrates, rank 38 of the scored kinases) and \gene{WNK3} was comparably suppressed ($z=-4.69$, $q=1.3\times10^{-5}$, 208 substrates). Because this estimate is motif-assigned over the full quantified phosphosite background, it extends the targeted WNK result rather than restating it. The same screen diverged from the targeted SPAK/OSR1 estimate: SPAK/STK39 (atlas label STLK3) and OSR1 substrate outputs were slightly positive ($z=+1.44$ over 302 substrates and $+1.57$ over 221 substrates, both n.s.) rather than suppressed, and \gene{WNK4} was suppressed only nominally ($q=0.11$). The unbiased atlas therefore corroborates the WNK component but not the SPAK/OSR1 arm; SPAK/OSR1 suppression should be read as targeted-substrate evidence from three quantified curated substrates, not as an unbiased kinome-wide finding.

Set against the five-cohort recurrence meta-analysis, the matched phosphoproteomic layer is where the transporter lesion is clearest. The recurrent RNA signal is carried mainly by the matrix/ECM-high set (Stouffer $z=3.39$), whereas the DCT/NCC-WNK transport decrease is weak and heterogeneous across cohorts ($z=-1.32$) and resolves into a defined effect only at the OSD-462 phosphosites.
...
\subsection{The bulk DCT-low signal partly reflects compartment remodeling}

Marker-panel analysis bore on whether the bulk DCT-low signal reflects transcriptional suppression, segment loss, or compartment dilution: DCT identity and DCT transport markers fell together across cohorts, while endothelial and stromal/fibroblast scores rose. Such compartment shifts can dilute per-cell DCT signals in bulk RNA---as interstitial and endothelial cells expand, they capture a larger share of the sequenced transcripts, lowering apparent DCT-gene levels even when surviving DCT cells transcribe normally---so a bulk DCT-low signal need not imply DCT-cell-intrinsic suppression. Within OSD-462, endothelial score increased in flight ($+1.00$, $p=0.036$) and stromal score increased directionally ($+0.67$, $p=0.054$). The endothelial score anti-correlated strongly with NCC regulatory phosphorylation across animals ($\rho=-0.762$, $p=0.0004$; Fig.~\ref{fig:endoscatter})---the strongest animal-level relationship in the dataset, which we decompose below---while DCT identity correlated positively with NCC regulatory phosphorylation ($\rho=+0.52$, $p=0.020$).

\begin{figure}[H]
\centering
\includegraphics[width=0.62\textwidth]{v11_endothelial_ncc_scatter.pdf}
\caption{\textbf{Per-animal endothelial score versus NCC regulatory phosphorylation in OSD-462} ($n=20$). Flight animals (red) carry higher endothelial/remodeling RNA scores and lower NCC regulatory phosphorylation, ground controls (blue) the reverse (Spearman $\rho=-0.762$, $p=0.0004$). This is the strongest animal-level relationship in the dataset and motivates the covariance decomposition; it is correlational and bulk-tissue, not a causal or cell-resolved measurement. The relationship is strongest across the flight/control contrast; within condition it is carried by the ground-control animals (ground control $\rho=-0.73$, flight $\rho=-0.20$), consistent with restricted range at $n=10$ per group.}
...
The per-animal OSD-462 data are compatible with a remodeling-linked transporter hypothesis, while also allowing parallel flight effects on tissue remodeling and transporter phosphorylation. The two cannot be separated from cross-sectional bulk data: distinguishing them requires spatial or time-course measurements of whether phosphorylation loss emerges in or near remodeling-rich neighborhoods (remodeling-linked) or independently of local matrix/endothelial expansion (parallel). Temporal order therefore remains an experimental question, since the present models are cross-sectional, small-sample, and based on bulk tissue.
...
The second constraint is translational. In the NASA Twins flight subject, urine and clinical-chemistry directions collapsed onto three physiological axes---aldosterone/fluid handling, water balance, and DCT transport---and all three were directionally aligned with the mouse-derived predictions. The signal is preliminary rather than decisive: it is a single flight trajectory, the exact two-sided by-axis sign test was not significant ($p=0.25$), and the \gene{AQP2} direction was read from a published figure rather than a numerical table. What would convert it into validation is explicit---machine-readable inflight human distal-nephron or urine-proteomic measurements across multiple subjects, against which the same three-axis prediction could be tested quantitatively---and OSD-656, as post-flight urine-inflammation context, is not yet such a measurement. The LINCS/CMap screen (50 up / 50 down query, 473,647 scored Level-5 signatures after \code{sig\_info} merge) is retained only to generate perturbagen-class hypotheses for future countermeasure work; as a cell-line, symbol-matched approximation it is not evidence for the present model. Detailed tables are in the companion compendium.
...
\node[layer, fill=green!12, below=of prot] (phos) {\textbf{Phosphorylation (OSD-462):} NCC/SPAK/WNK regulatory sites $\downarrow$ --- the transporter lesion};
...
\draw[->, thick] (drv) -- (phos);
\end{tikzpicture}
\caption{\textbf{Model: the spaceflight kidney response is layer-resolved.} The recurrent bulk-RNA remodeling state does not propagate to protein abundance (the matrix/ECM set is inverted at protein), so the RNA layer is a tissue-context readout. The transporter lesion is carried by NCC/SPAK/WNK regulatory phosphorylation, which we propose is driven by reduced WNK--SPAK activity downstream of the microgravity fluid shift---through the volume/aldosterone axis, the chloride/voltage potassium switch, or both. The driver arrow is a hypothesis; the layer pattern is the result.}
\label{fig:model}
\end{figure}

\subsection{Main interpretation}

Across mouse kidney spaceflight cohorts, the dominant recurring RNA component was matrix/endothelial remodeling, accompanied by directional but weaker DCT/NCC-WNK transport suppression (Fig.~\ref{fig:model}). In OSD-462/RR-10 multi-omics, that RNA state did not behave as a protein-abundance program: RNA and protein were decoupled at genome scale (Spearman $-0.034$), no curated set propagated cleanly to protein abundance, targeted protein concordance was null, the one null-tested set (matrix/ECM) was inverted, and DCT/NCC-WNK protein abundance was slightly positive while RNA was lower. The transporter-regulatory signal was sharpest at the phosphorylation layer.

Reduced NCC regulatory phosphorylation against flat total abundance indicates reduced NCC activation and lower distal sodium-chloride reabsorption; through the well-characterized coupling of the distal nephron, this propagates to potassium, calcium, and magnesium handling. These public omics lack matched urine chemistry, serum electrolytes, blood pressure, and transporter-activity readouts, so what remains to be measured is the magnitude of the shift in flight, not the existence of the chain.

Cosmic Kidney Disease established spaceflight-associated NCC/distal-nephron dephosphorylation in RR-10/OSD-462 \citep{siew2024cosmic}; the present analysis shows that the RNA, protein, and phosphoprotein layers \emph{disagree}---that the recurrent matrix/endothelial RNA state does not propagate to protein abundance and that the transporter signal is concentrated in regulatory phosphorylation---and ranks the suppressed phosphosites against a DCT1/DCT2 subtype prior. The result is non-obvious: the RNA and protein layers systematically diverge while phosphorylation aligns with the established endpoint. The dephosphorylation sits inside a recurrent matrix/endothelial-high RNA state that does not propagate to protein abundance, and an unbiased kinome-wide screen corroborates the WNK arm while leaving SPAK/OSR1 as targeted-substrate evidence. The transporter lesion is therefore a phosphorylation-layer event embedded in tissue remodeling, not a transcriptional or abundance program, and phosphorylation rather than abundance is the layer that future spaceflight kidney studies should measure directly.

\subsection{The distal-nephron subtype-prior signal and the upstream mechanism}

The DCT-prior result refines the distal-nephron interpretation by separating parent-gene prioritization from cell-of-origin assignment. OSD-462 is whole-kidney phosphoproteomics, and GSE228367 is an external transcriptomic reference. Their intersection shows that the parent genes of flight-suppressed phosphosites are overrepresented in distal-nephron subtype-prior extremes, including DCT1-high and DCT2-leaning bins.

The DCT1 top-decile subset remains biologically important because NCC/SPAK/WNK anchors are DCT1-leaning in the reference. \gene{Slc12a3}, \gene{Stk39}, and \gene{Wnk4} fall in the DCT1 top decile under the primary mean-difference score, and leave-one-replicate-out DCT1-top and DCT2-leaning decile bins are stable for that score definition (Jaccard approximately 0.85--0.86). The DCT2-leaning bin is also enriched and is stronger in several parent-gene-aware summaries, placing the result in a broader distal-nephron subtype-prior frame. Parent-gene Fisher, cluster bootstrap, matched permutation, threshold-free, and parent-protein/composition-aware analyses reduce the simplest row-amplification and abundance explanations, while the parent-gene logistic and matched-permutation results keep the interpretation at parent-gene enrichment rather than DCT1 exclusivity or cellular localization. Consistent with the Results, the DCT1-high bin is strong at the row level but does not survive the matched permutation ($p=0.107$) while the DCT2-leaning bin does, so the supported claim is distal-nephron subtype-prior enrichment led by the DCT2-leaning extreme, with DCT1 biologically anchored but more conditional. A direct test would require DCT-enriched, spatially resolved, or segment-specific spaceflight phosphoproteomics.

The DCT2-leaning result is the more provocative half: the canonical NCC/SPAK/WNK anchors are DCT1-leaning, yet the DCT2-leaning bin is the one that survives observability-matched permutation. The bin's specific end contains canonical DCT2/CNT and aldosterone-sensitive markers (\gene{Trpv5}, \gene{Calb1}, \gene{Scnn1b/g}, \gene{Nedd4l}, \gene{Sgk1}; Table~\ref{tab:dct2validation}), but because it is defined by the lowest DCT1 score it also contains broadly expressed low-DCT1 genes; the DCT2/CNT-program reading below is therefore a biological hypothesis layered on a low-DCT1 statistical bin, not a cell-type assignment. One reading is that flight suppresses a phosphoregulatory program of the aldosterone-sensitive distal nephron that is broader than NCC itself. The DCT2/CNT transition is where the SGK1--NEDD4L axis tunes ENaC, where the calcium-transport machinery (\gene{Trpv5}, \gene{Calb1}) is regulated, and where WNK--SPAK signaling also acts on ROMK (\gene{Kcnj1}) and the K--Cl cotransporters \citep{subramanya2014dct,richardson2008wnk}; loss of regulatory phosphorylation on these DCT2/CNT-expressed substrates would register as DCT2-leaning enrichment without requiring NCC itself to be DCT2-specific. That would broaden the phenotype from sodium-chloride reabsorption alone toward coupled potassium and calcium handling in the aldosterone-sensitive segment---a hypothesis that DCT-subtype-resolved phosphoproteomics could test directly by asking which DCT2/CNT kinase substrates lose phosphorylation in flight.
...
The endothelial and matrix/endothelial covariance-decomposition summaries formalize an observation that is biologically interesting on its own: animals with higher endothelial/remodeling scores tended to have lower NCC regulatory phosphorylation. The summaries are compatible with a remodeling-linked transporter hypothesis and with parallel flight effects on tissue remodeling and transporter phosphorylation. This distinction defines an experimental fork. If remodeling precedes transporter deactivation, spatial and time-course data should show DCT phosphorylation loss emerging in or near remodeling-rich neighborhoods. If the processes are parallel, DCT phosphoregulation should change without requiring local matrix/endothelial expansion. The external IRI repair reference offers only a weak prior: there, the DCT-transport drop concentrated in DCT-adjacent rather than DCT-core spots, which---if spaceflight shares that geometry---would lean toward the remodeling-linked branch, though IRI is not spaceflight and the prior is suggestive at best.
...
The analysis uses public datasets with heterogeneous missions, strains, sex, and protocols; RRRM-2 is small for age-by-flight interactions and OSD-253 has imperfect controls. Critically, these records lack matched physiology---urine and serum electrolytes, blood pressure---so the molecular pattern cannot be connected to kidney function here.

OSD-462 phosphoproteomics is whole-kidney and cannot assign phosphosites to DCT cells; the DCT prior is snRNA-seq, so the analysis prioritizes parent genes rather than localizing sites. Phosphosite rows are not fully independent, and the $p<0.05$ set is a directional enrichment set, not per-site significance; parent-gene-aware, cluster-bootstrap, matched-permutation, and site-count-stratified analyses reduce but do not remove this structure, and under matched permutation DCT1 attenuates while DCT2 remains stable. Composition-aware models use correlated bulk RNA scores and may overadjust if remodeling lies on the causal path, and the parent-protein-normalized re-test is whole-kidney effect normalization, not biochemical stoichiometry. Channel-median centering may also remove part of a true global phosphorylation shift, so the primary result is interpreted as subtype-prior enrichment of suppressed sites rather than an estimate of global phosphorylation loss.

The covariance decomposition is cross-sectional and small-sample; spatial references are IRI rather than spaceflight; and the perturbation references narrowed but did not resolve the upstream signal (the low-K effect was confined to a 14-gene subset with wide intervals, dDAVP shared no transport-target sites, and KLHL3/CUL3 turnover was untestable for lack of \gene{Klhl3} coverage and kidney ubiquitinomics). The human and CMap layers are exploratory: a single Twins trajectory (by-axis sign test $p=0.25$, figure-level \gene{AQP2}) and a cell-line CMap approximation retained only for perturbagen-class hypothesis generation.

\subsection{Open novelty extensions}

Two layer-specificity extensions are scoped but deliberately deferred. The first is a formal cell-type deconvolution (MuSiC/Bisque) of the four cohorts with residualization of pathway RNA effects on estimated cell fractions, which would convert the current marker-panel composition reading into a quantitative split between intrinsic regulation and dilution. The phospho-layer composition question is already resolved---99.3\% of M0-suppressed sites stay negative after the M0$\to$M4 composition-aware adjustment ladder---so the remaining gap is the RNA pathway layer rather than the phosphosite endpoint. The second is a non-kidney negative-control tissue contrast against OSD-105 (RR-1 tibialis anterior muscle, the strongest publicly available matched RNA/protein non-kidney mouse spaceflight cohort), which would elevate the discordance from a ``kidney shows X'' statement to a kidney-distal-nephron specificity claim, conditional on confirming TMT-versus-label-free compatibility against the OSD-462 anchor. Both restart paths, including their pre-registered nulls, are documented in \path{docs/v11_layer_specificity_execution_summary_2026-06-07.md}.

\subsection{Future experiments}

Next experiments follow directly from this model, and they are not of equal priority. The decisive experiment is DCT-enriched or spatially resolved spaceflight phosphoproteomics, which would test whether the DCT-subtype-prior parent-gene subset localizes to DCT1, DCT2/CNT, DCT-adjacent compartments, or a broader distal-nephron neighborhood. Spatial transcriptomics on flight kidney sections should determine whether DCT-low regions lie near endothelial, fibro-inflammatory, or injured-tubule repair niches, with the specific IRI-derived prediction that DCT-adjacent spots will carry the DCT-transport drop while DCT-marker-high spots may retain core DCT identity. Lower in priority, because they probe one candidate mechanism rather than the central localization claim, ubiquitinomics and KLHL3/CUL3/WNK turnover assays would distinguish enhanced WNK degradation from ionic or osmotic suppression of WNK-SPAK activity; the public datasets lack \gene{Klhl3} phosphosite coverage and kidney ubiquitinomics for this test. Animal-matched electrolyte physiology, urine and serum chemistry, blood pressure, and NCC abundance/phosphorylation would connect the molecular pattern to kidney function. A targeted electrolyte or ionic perturbation in flight or analog studies would test the directional low-K-opposing signal suggested by the public GSE228367 reference.
...
\bibitem{spacekidney2023} % AUTHORS: confirm full list from doi:10.1038/s41526-023-00268-1
[Author list to confirm]. The kidney, volume homeostasis and osmoregulation in space: current perspective and knowledge gaps. \emph{npj Microgravity}. 2023;9:29. doi:10.1038/s41526-023-00268-1.
