...
\title{\textbf{Layer-resolved mouse-kidney multi-omics implicate distal-nephron phosphoregulatory suppression after spaceflight}\\[0.35em]
\large Cross-cohort RNA recurrence and matched RR-10/OSD-462 multi-omics place the transporter signal at phosphorylation, beyond the canonical NCC/DCT1 axis}
...
\begin{abstract}
\noindent
Spaceflight dephosphorylates the sodium-chloride cotransporter NCC and its WNK--SPAK/OSR1 regulatory kinases in the kidney, but whether the accompanying transcriptomic response reflects, contradicts, or simply misses that endpoint had not been resolved. Analyzing public mouse kidney RNA-seq, TMT protein, and phosphoproteomic datasets (RRRM-2/OSD-771, OSD-513, OSD-253, and matched RR-10/OSD-462) against external DCT1/DCT2 single-nucleus, perturbation, spatial, and human-urine references, we find that this transporter response is largely post-translational. In bulk RNA it is weak and cohort-dependent---the robust, recurrent transcriptomic signal is instead a nonspecific matrix/endothelial remodeling program---and it is absent from total protein abundance, resolving specifically at NCC/SPAK/WNK regulatory phosphosites; an unbiased kinome-wide screen independently recovers this suppression as WNK1 and WNK3, with parent-gene enrichment pointing beyond the canonical NCC/DCT1 axis.

Across independent RNA cohorts, flight raised matrix/endothelial remodeling and lowered DCT/NCC-WNK transport scores; remodeling recurred robustly in a five-cohort meta-analysis ($p=7.0\times10^{-4}$), while transport suppression was more cohort-dependent and resolved most clearly at phosphorylation. In matched OSD-462 multi-omics the RNA program did not reach protein: RNA and protein flight effects were decoupled at genome scale (Spearman $-0.034$, the loose coupling expected for bulk tissue), and the one curated set with a null-tested protein effect, matrix/ECM, showed a suggestive protein-level inversion ($q=0.091$) rather than concordance. Against that flat protein background, NCC/SPAK/WNK regulatory phosphosites fell, and an unbiased kinome-wide screen independently recovered WNK1 ($q=5.4\times10^{-6}$) and WNK3 ($q=1.3\times10^{-5}$) suppression from the full phosphosite background.

Ranking the parent genes of nominally flight-suppressed phosphosites against the DCT1/DCT2 prior placed the suppression at distal-nephron subtype-prior extremes, most robustly in a DCT2/CNT--leaning bin that held under an observability-matched parent-gene permutation and parent-protein normalization. Because the phosphoproteomics is whole-kidney, this is parent-gene prioritization rather than cell-resolved localization; within that limit it is consistent with involvement of aldosterone-sensitive distal-nephron programs beyond the canonical NCC/DCT1 axis, and external perturbation and spatial references nominated DCT-adjacent remodeling neighborhoods as the spatial hypothesis to test.

The spaceflight kidney transcriptome is therefore a tissue-context readout that does not by itself report transporter state; DCT-subtype-resolved or spatial phosphoproteomics, not additional bulk transcriptomics, is a priority next experiment, paired with electrolyte and blood-pressure physiology, to localize and functionally test the suppression.
\end{abstract}
...
\begin{tikzpicture}[>=Stealth, font=\small,
  layer/.style={draw, rounded corners, align=left, text width=72mm, minimum height=14mm, inner sep=5pt},
  chip/.style={draw, rounded corners, align=center, text width=40mm, minimum height=14mm, inner sep=3pt, font=\footnotesize},
  drv/.style={draw, rounded corners, align=center, fill=orange!12, text width=72mm, inner sep=5pt, font=\footnotesize},
  cor/.style={draw, rounded corners, align=left, fill=green!6, text width=64mm, inner sep=4pt, font=\footnotesize},
  banner/.style={draw, rounded corners, align=center, fill=blue!5, text width=140mm, inner sep=5pt},
  hdr/.style={font=\footnotesize\bfseries, align=center}]
\node[draw, rounded corners, fill=black!6, align=center, font=\footnotesize, text width=72mm] (input)
  {\textbf{Spaceflight mouse kidney}\\[-1pt]{\scriptsize RRRM-2/OSD-771, OSD-513, OSD-253, RR-10/OSD-462}};
\node[layer, fill=blue!8, below=7mm of input] (rna)
  {\textbf{Transcriptome}\,\,{\scriptsize bulk RNA-seq, 5 cohorts}\\[1pt]
   {\scriptsize Robust: matrix/endothelial remodeling $\uparrow$ (meta $p{=}7.0{\times}10^{-4}$)}\\[-1pt]
   {\scriptsize Transporter (DCT/NCC--WNK) $\downarrow$ --- weak, cohort-dependent}};
\node[layer, fill=gray!12, below=11mm of rna] (prot)
  {\textbf{Proteome}\,\,{\scriptsize TMT, OSD-462}\\[1pt]
   {\scriptsize Decoupled from RNA (Spearman $-0.034$)}\\[-1pt]
   {\scriptsize Total NCC / transporters flat; matrix/ECM inverted ($q{=}0.091$)}};
\node[layer, fill=green!14, below=7mm of prot] (phos)
  {\textbf{Phosphoproteome}\,\,{\scriptsize OSD-462}\\[1pt]
   {\scriptsize NCC / SPAK / WNK regulatory phosphosites $\downarrow$}\\[-1pt]
   {\scriptsize \textbf{the transporter-regulatory lesion}}};
\node[drv, below=9mm of phos] (drv)
  {\textbf{Proposed driver} {\scriptsize(hypothesis)}\\[1pt]
   cephalad fluid shift $\rightarrow$ RAAS/aldosterone $+$ Cl$^-$/voltage WNK switch\\[-1pt]
   $\rightarrow$ $\downarrow$WNK--SPAK drive $\rightarrow$ NCC dephosphorylation};
\node[chip, fill=blue!5, right=10mm of rna] (frna) {\emph{weak}\\cohort-dependent};
\node[chip, fill=gray!6, right=10mm of prot] (fprot) {\emph{flat}\\NCC unchanged};
\node[chip, fill=green!22, draw=green!45!black, very thick, right=10mm of phos] (fphos) {$\downarrow$~\textbf{suppressed}\\resolved here};
\node[hdr, above=3mm of frna] (fatehdr) {NCC/WNK transporter signal};
\node[cor, below=9mm of fphos] (cor)
  {\textbf{Corroboration \& localization}\\[1pt]
   Kinome screen: WNK1 ($q{=}5.4{\times}10^{-6}$), WNK3 ($q{=}1.3{\times}10^{-5}$) $\downarrow$\\[1pt]
   Parent genes \textbf{DCT2/CNT-leaning, beyond canonical NCC/DCT1} (matched-perm $p{=}0.017$)};
\node[banner] at ($(drv.south west)!0.5!(cor.south east) + (0,-9mm)$) (banner)
  {\textbf{Bulk transcriptomics does not report transporter state} --- the spaceflight transporter deficit is a phosphorylation-layer event carried by NCC/SPAK/WNK regulatory sites.};
\draw[->, thick] (input) -- (rna);
\draw[->, red!70!black, thick] (rna) -- node[right, font=\scriptsize, text=red!70!black]{does \emph{not} propagate} (prot);
\draw[->, thick] (prot) -- (phos);
\draw[->, thick, orange!75!black] (drv) -- node[right, font=\scriptsize]{drives} (phos);
\draw[->, gray!55, thick] (frna) -- (fprot);
\draw[->, green!45!black, thick] (fprot) -- (fphos);
\draw[->, green!45!black, dashed] (fphos) -- (cor);
\end{tikzpicture}
\caption{\textbf{The spaceflight kidney response is layer-resolved: the transporter deficit is a phosphorylation-layer event.} One perturbation (spaceflight) read out across three molecular layers of the same RR-10/OSD-462 mice, with independent RNA cohorts anchoring the transcriptome. The robust, recurrent bulk-RNA signal is nonspecific matrix/endothelial remodeling (five-cohort meta $p=7.0\times10^{-4}$), and it does not propagate to protein abundance (RNA--protein decoupled at genome scale, Spearman $-0.034$; the one null-tested set, matrix/ECM, inverted at protein, $q=0.091$). The NCC/WNK transporter signal (right) is weak and cohort-dependent in RNA, flat in total protein, and resolves only at regulatory phosphorylation, where NCC/SPAK/WNK sites fall; an unbiased kinome-wide screen independently recovers WNK1 ($q=5.4\times10^{-6}$) and WNK3 ($q=1.3\times10^{-5}$), and the suppressed phosphosites' parent genes are DCT2/CNT-leaning, beyond the canonical NCC/DCT1 axis (observability-matched permutation $p=0.017$). The proposed upstream driver (orange) is a hypothesis; the layer pattern is the result.}
\label{fig:model}
\end{figure}
...
This is what bulk kidney RNA-seq cannot settle on its own. A whole-kidney transcriptome reports which cell types are present as much as how each cell regulates transport, so a recurring pattern of higher matrix/endothelial and lower DCT/NCC-WNK signal is equally compatible with DCT transcriptional suppression, DCT segment loss, or dilution by expanding interstitial and endothelial compartments \citep{avilacobos2020deconv,nido2020composition}. Even before this compositional problem, transcript and protein abundance are only loosely coupled in tissue \citep{liu2016dependency}; such a pattern therefore need not correspond to any change in transporter protein abundance or activation state. We treat the RNA--protein mismatch below as the expected inference problem for bulk tissue, not as the standalone novelty. The discriminating observation is whether matched protein and phosphorylation layers resolve the transporter phenotype when bulk RNA does not. We accordingly combine cross-cohort RNA recurrence with OSD-462 protein and phosphoprotein contrasts and an external native DCT1/DCT2 single-nucleus reference.
...
Read this way, the spaceflight kidney transporter phenotype is not a transcriptional or abundance program but a regulatory-phosphorylation one, and its reach across the distal nephron is wider than the canonical NCC/DCT1 axis. The parent genes of flight-suppressed phosphosites are enriched at distal-nephron subtype-prior extremes, most robustly at the DCT2/CNT--leaning end that overlaps the aldosterone-sensitive distal nephron, including SGK1--NEDD4L--ENaC regulation and calcium-handling markers such as \gene{Trpv5} and \gene{Calb1} \citep{arroyo2011aldosterone,subramanya2014dct,su2024dct}. This reframes the spaceflight distal-nephron phenotype from a narrow NCC/DCT1 event toward a broader phosphoregulatory suppression of the aldosterone-sensitive distal nephron---a hypothesis that the matched layers raise and that segment-resolved phosphoproteomics can test directly.
...
\begin{table}[H]
\centering\small
\caption{\textbf{Datasets and analysis roles.} All flight contrasts are Space Flight minus Ground Control. Per-arm sample sizes follow the OSDR submissions; the matched OSD-462 flight-versus-control comparison uses 20 animals. Dashes mark fields not fixed by the present analysis.}
\label{tab:datasets}
\begin{tabularx}{\textwidth}{@{}lp{2.5cm}cp{2.3cm}Y@{}}
\toprule
Dataset (OSDR) & Strain & Sex & Omics & Role \\
\midrule
RRRM-2 / OSD-771 & C57BL/6NTac (young, old) & F & RNA & Primary RNA anchor; ISS-terminal and live-return arms \\
OSD-513 & C57BL/6J (young) & M & RNA & Independent recurrence and sex-robustness cohort \\
OSD-253 & C57BL/6J, C3H/HeJ & --- & RNA & Strain and TLR4 mechanism context \\
OSD-102 & C57BL/6J (young) & F & RNA & Cross-cohort recurrence meta-analysis \\
OSD-163 & BALB/c background & --- & RNA & Cross-cohort recurrence meta-analysis \\
RR-10 / OSD-462 & B6129SF2/J & F & RNA, protein, phospho & Matched multi-omic anchor ($n=20$ flight + ground control) \\
\bottomrule
\end{tabularx}
\end{table}

\subsection{Analysis hierarchy and endpoints}

The analyses fall into three tiers, fixed before the flight contrasts were computed. \emph{Primary endpoints:} (i) cross-cohort RNA recurrence of the matrix/endothelial-high, DCT/NCC-WNK-low signature; (ii) the matched OSD-462 RNA-versus-protein-versus-phosphosite comparison; and (iii) DCT1/DCT2 parent-gene enrichment of nominally suppressed phosphosites. \emph{Secondary, robustness:} composition-aware phosphosite models, parent-protein normalization, targeted and kinome-wide KSEA, and TMT channel-centering sensitivity. \emph{Exploratory, hypothesis-generating:} the covariance decomposition, the low-potassium and dDAVP/vasopressin perturbation references, the ischemia-reperfusion spatial atlas, KLHL3/CUL3 coverage, the mineralocorticoid/aldosterone axis, human urine concordance, CMap, aging, live-return, and network analyses. The curated gene sets and the DCT1/DCT2 percentile bins were specified before the flight contrasts; exploratory results are reported as context and do not feed the primary claim.
...
Calibrating these direction calls against the matched null sharpened them into explicit layer assignments and recovered both pre-registered endpoints (Methods). Matrix/ECM and TLR4/innate emerged as suggestive calibrated protein inversions, the protein layer trending opposite the RNA direction against the abundance- and peptide-count-matched null (matrix/ECM signed-mean protein $-0.107$, inverse-signed-mean BH $q=0.091$; TLR4/innate inverse-slope BH $q=0.078$). A further three pathways behaved as RNA-to-phospho carries, their RNA direction reappearing at the phosphosite parent mean rather than at protein abundance: DCT/NCC-WNK transport (31\% of RNA-directional members carried at phospho, $n=13$), broad tubular transport, and S1P/S1PR3. The pre-registered matrix/ECM inversion and DCT/NCC-WNK phospho-carry were thus both recovered, and the three carry candidates extend the pattern beyond what was hypothesized (Fig.~\ref{fig:propagation}; Benjamini-Hochberg within the 11-pathway family).
...
Nominally flight-suppressed phosphosite rows (negative effect, $p<0.05$) are not a random sample of the kidney phosphoproteome: they concentrate in the parent genes that define distal-nephron identity, and the signal that holds under the most stringent observability-matched permutation is the DCT2-leaning extreme rather than the canonical DCT1 anchors. The sensitivity ladder establishing this follows.
...
The DCT2-leaning bin strengthens where DCT1 attenuates. Defined as the lowest-DCT1-score decile, it was enriched at the row level (OR 1.29, q=$1.30\times10^{-5}$), after composite-site exclusion (OR 1.33, q=$1.30\times10^{-5}$), in the one-row-per-parent-gene reduction (OR 1.68, q=$4.62\times10^{-6}$), in the stricter single-position reduction (OR 1.52, q=$2.91\times10^{-4}$), at parent-gene level (OR 1.82, q=$3.29\times10^{-8}$), and under the same matched parent-gene permutation that DCT1 fails (empirical $p=0.017$). Joint logistic models including both decile flags attenuated each nominal endpoint (DCT1 OR 1.17, q=0.320; DCT2 OR 1.27, q=0.104), so the supported claim is enrichment across the distal-nephron subtype prior rather than exclusivity at either pole. The DCT1 top decile remains biologically motivated because NCC, SPAK, and WNK4 are DCT1-leaning by the GSE228367 mean-difference prior. The DCT2/CNT transition is also the entry into the aldosterone-sensitive distal nephron, where SGK1--NEDD4L regulation of ENaC and distal calcium-handling markers such as \gene{Trpv5} and \gene{Calb1} are prominent \citep{arroyo2011aldosterone,subramanya2014dct,su2024dct}; this makes the DCT2-leaning bin biologically plausible in a broad distal-transport phenotype, even though magnesium handling through \gene{Trpm6} remains DCT1-leaning in the reference. Table~\ref{tab:dct2compare} and Fig.~\ref{fig:dctladder} summarize the DCT1/DCT2 evidence ladder. Spaceflight therefore acts selectively on the regulatory phosphorylation of the distal-transport program rather than diffusely across kidney signaling, which is what allows a whole-kidney phosphoproteome to implicate a specific nephron segment.
...
Two references that might have named an upstream mechanism were coverage-limited, but the gap is itself informative. The PXD001729 dDAVP/mpkDCT phosphoproteome shared 60 single sites with OSD-462 but none of the 14 transport targets, and KLHL3/CUL3-mediated WNK turnover could not be probed (\gene{Klhl3} had zero quantified phosphosites, \gene{Cul3} one, and no public kidney ubiquitinomics exist). The salient result is the negative one: WNK, SPAK, and NCC protein abundances were flat to slightly positive, which argues against a degradation-driven loss of these proteins and points instead toward ionic/osmotic suppression of WNK--SPAK \emph{activity}---the same direction implied by the low-K reference.
...
% (Graphical-abstract model figure promoted to the Introduction as Figure~\ref{fig:model}.)

\subsection{Main interpretation}

The central result is that the spaceflight kidney transporter signal is carried at regulatory phosphorylation, not at transcript or total-protein abundance, and that the bulk transcriptome does not report it. Across mouse kidney spaceflight cohorts the dominant recurring RNA component was matrix/endothelial remodeling, with weaker DCT/NCC-WNK transport suppression (Fig.~\ref{fig:model}); in OSD-462/RR-10 multi-omics that RNA state did not behave as a protein-abundance program (RNA and protein decoupled at genome scale, Spearman $-0.034$; targeted protein concordance null; the one null-tested set, matrix/ECM, inverted; DCT/NCC-WNK protein flat-to-positive while RNA fell). The transporter-regulatory signal was sharpest at the phosphorylation layer.

Reduced NCC regulatory phosphorylation against flat total abundance indicates reduced NCC activation and predicted lower distal sodium-chloride reabsorption; through the well-characterized coupling of the distal nephron, the same logic extends to potassium, calcium, and magnesium handling. The molecular chain from regulatory phosphorylation to distal electrolyte balance is therefore supported.

Cosmic Kidney Disease established spaceflight-associated NCC/distal-nephron dephosphorylation in RR-10/OSD-462 \citep{siew2024cosmic}; on the same animals, the present analysis adds what that endpoint alone could not show. The RNA, protein, and phosphoprotein layers show \emph{limited concordance}: the recurrent matrix/endothelial RNA state fails to reach protein abundance while the transporter signal concentrates in regulatory phosphorylation, which we then rank against a DCT1/DCT2 subtype prior. This divergence is itself the central finding: the RNA and protein layers systematically diverge while phosphorylation aligns with the established endpoint, and an unbiased kinome-wide screen independently corroborates the WNK arm. The transporter-regulatory deficit is thus a phosphorylation-layer event embedded in tissue remodeling, not a transcriptional or abundance program, and phosphorylation is the layer future spaceflight kidney studies should measure directly.

\begin{table}[H]
\centering\small
\caption{\textbf{From molecular result to predicted physiology.} The phosphorylation-layer findings imply distal-transport changes this study does not measure directly. Confidence reflects how directly the molecular result constrains the physiology; the last column lists the measurement that would test it.}
\label{tab:physpred}
\begin{tabularx}{\textwidth}{@{}YYlY@{}}
\toprule
Molecular result & Predicted physiology & Confidence & Validation needed \\
\midrule
$\downarrow$ NCC N-terminal and SPAK regulatory phosphosites ($-0.8$ to $-1.6$ log$_2$) & $\downarrow$ NCC activation & High & pNCC immunoblot / immunostaining \\
Total NCC protein unchanged ($+0.09$) & Regulation phosphorylation-, not abundance-driven & Mod.--high & Targeted proteomics / western \\
Kinome-wide WNK1/WNK3 output suppressed & Reduced WNK--SPAK drive & Moderate & DCT kinase-activity / pSPAK readouts \\
DCT2/CNT-leaning parent-gene enrichment (\gene{Wnk1}, \gene{Nedd4l}, \gene{Nr3c2}, \gene{Scnn1g}, \gene{Slc8a1}) & Possible aldosterone-sensitive distal-nephron involvement & Low--mod. & Segment-resolved phosphoproteomics \\
\gene{Trpv5}/\gene{Calb1} in bin but no suppressed phosphosites & Calcium-handling change unresolved (direction uncertain) & Low & Urine Ca/Mg, TRPV5/calbindin staining \\
Endothelial/remodeling score anti-correlates with NCC phosphorylation & Remodeling-linked suppression possible & Low--mod. & Spatial / time-course data \\
\bottomrule
\end{tabularx}
\end{table}
...
Although the canonical NCC/SPAK/WNK anchors are DCT1-leaning, it is the DCT2-leaning bin that survives observability-matched permutation. The bin's specific end contains canonical DCT2/CNT and aldosterone-sensitive markers (\gene{Trpv5}, \gene{Calb1}, \gene{Scnn1b/g}, \gene{Nedd4l}, \gene{Sgk1}; Table~\ref{tab:dct2validation}), but because it is defined by the lowest DCT1 score it also contains broadly expressed low-DCT1 genes; the DCT2/CNT-program reading below is therefore a biological hypothesis layered on a low-DCT1 statistical bin, not a cell-type assignment. One reading is that flight suppresses a phosphoregulatory program of the aldosterone-sensitive distal nephron that is broader than NCC itself. Segment physiology makes that reading plausible: the DCT2/CNT transition contains SGK1--NEDD4L--ENaC regulation, calcium-transport machinery marked by \gene{Trpv5} and \gene{Calb1}, and WNK-sensitive potassium/chloride handling \citep{arroyo2011aldosterone,subramanya2014dct,terker2015potassium,su2024dct}. Loss of regulatory phosphorylation on DCT2/CNT-expressed substrates would therefore register as DCT2-leaning enrichment without requiring NCC itself to be DCT2-specific. That would broaden the phenotype from sodium-chloride reabsorption alone toward coupled sodium, potassium, and calcium handling in the aldosterone-sensitive segment---a hypothesis that DCT-subtype-resolved phosphoproteomics could test directly by asking which DCT2/CNT kinase substrates lose phosphorylation in flight.

The leading edge of this bin is, however, mostly non-canonical. Of the 186 DCT2-leaning parent genes carrying a nominally suppressed phosphosite, five are canonical aldosterone-sensitive distal-nephron genes---\gene{Wnk1}, \gene{Nedd4l}, the mineralocorticoid receptor \gene{Nr3c2}, ENaC-$\gamma$ (\gene{Scnn1g}), and \gene{Slc8a1}/NCX1 (Table~\ref{tab:leadingedge})---while the largest-magnitude suppressed sites fall on broadly expressed, low-DCT1 cytoskeletal and scaffolding proteins. The calcium machinery is therefore an implicated segmental context rather than a demonstrated phosphosite phenotype: \gene{Trpv5} and \gene{Calb1} sit in the bin by expression but contribute no suppressed phosphosites, and spaceflight calcium balance is itself shaped by hypercalciuria and stone-risk physiology \citep{smith2014men,siew2024cosmic,chouker2025impact}. The direction of any DCT2/CNT calcium-handling change cannot be inferred from reduced distal phosphorylation without urine-calcium and TRPV5/calbindin measurements. The DCT2/CNT signal is therefore best read as the aldosterone/ENaC arm of the segment being represented among the suppressed phosphosites, with calcium handling retained as the physiological angle to test rather than as a resolved calcium-transport phenotype.
...
These results refine how a bulk RNA spaceflight kidney signal should be interpreted. Because the recurrent matrix/endothelial remodeling transcription does not track protein abundance, and the transporter-regulatory deficit appears at NCC/SPAK/WNK regulatory phosphorylation, bulk RNA alone is insufficient to infer protein remodeling or transporter activation state: a remodeling RNA signature need not reflect protein-level remodeling, and it does not capture the regulatory-phosphorylation event. The practical implication is specific: spaceflight kidney studies should prioritize phosphoproteomic, and ideally DCT-subtype-resolved or spatial phosphoproteomic, measurements over additional bulk transcriptomics, and should pair them with the urine/serum electrolyte and blood-pressure physiology needed to connect reduced NCC activation to function. The most direct next test is DCT-enriched or spatially resolved phosphoproteomics on flight kidney: it would test whether the suppressed distal-transport program is DCT-intrinsic or concentrated in DCT-adjacent remodeling neighborhoods, and would clarify the DCT2-leaning signal, which is the most permutation-robust part of the enrichment.
...
\begin{table}[H]
\centering\small
\caption{\textbf{Canonical aldosterone-sensitive genes in the DCT2-leaning leading edge.} Of the 186 DCT2-leaning (lowest-DCT1-score decile) parent genes carrying at least one nominally suppressed phosphosite ($\hat{\beta}_{Fs}<0$, $p<0.05$), these five are canonical aldosterone-sensitive distal-nephron genes; the strongest suppressed site per gene is shown. The largest-magnitude suppressed sites in the bin instead fall on broadly expressed, low-DCT1 cytoskeletal/scaffolding genes (e.g.\ \gene{Limch1}, \gene{Lmo7}, \gene{Ablim1}, \gene{Epb41l1}, \gene{Kcnq1}). The calcium genes \gene{Trpv5} and \gene{Calb1} lie in the bin by expression but contribute no suppressed phosphosites, so the bin represents the aldosterone/ENaC arm of the segment rather than a calcium-transport phenotype.}
\label{tab:leadingedge}
\begin{tabular}{@{}llcl@{}}
\toprule
Parent gene & Role & Supp.\ sites & Strongest suppressed site \\
\midrule
\gene{Wnk1} & WNK kinase (distal nephron) & 3 & S2392 ($-0.63$, $p=4.4\times10^{-4}$) \\
\gene{Nedd4l} & ENaC ubiquitin ligase & 3 & S484 ($-0.41$, $p=3.0\times10^{-3}$) \\
\gene{Slc8a1} & NCX1, Ca$^{2+}$ extrusion & 1 & S389 ($-0.51$, $p=0.026$) \\
\gene{Nr3c2} & Mineralocorticoid receptor & 1 & S286/287 ($-0.23$, $p=0.031$) \\
\gene{Scnn1g} & ENaC $\gamma$-subunit & 1 & S628 ($-0.12$, $p=0.048$) \\
\bottomrule
\end{tabular}
\end{table}
