...
Within each Age $\times$ Arm $\times$ Environment-Group cell, residuals were standardized to zero mean and unit variance per gene before pooling for topology selection; the unstandardized residual matrix was used for downstream LIONESS edge-weight computation \citep{kuijjer2019estimating}. The shared partial-correlation skeleton was built via Ledoit--Wolf shrinkage covariance estimation \citep{ledoit2003honey}, precision-matrix inversion, off-diagonal extraction $\mathrm{pc}_{ij} = -P_{ij}/\sqrt{P_{ii}P_{jj}}$, and top-$k{=}80$ neighbor selection per gene. LIONESS sample-specific edge weights were computed using the Kuijjer linear-contribution formula on raw Pearson correlations bounded in $[-1, 1]$, $w_s = N \cdot r_{\text{all}} - (N-1) \cdot r_{-s}$, rather than in Fisher-$z$ space. The motivation is conceptual: the LIONESS output is a per-sample linear contribution to the aggregate network edge weight, not itself a correlation, and applying the Fisher-$z$ transform to LIONESS contributions treats them as if they were bounded in $[-1, 1]$ when they are not. We retain the Fisher-$z$ variant ($z_s = N \cdot z_{\text{all}} - (N-1) \cdot z_{\text{-s}}$ with pre-\code{atanh} clipping at $|r| \leq 0.9995$ and post-formula clipping at $|z| \leq 20$) as the \code{z\_contribution} sensitivity mode but use it only for cross-checks. Per-edge sample weights were rank-normalized across samples to unit-normal scores (\code{raw\_ranknorm} mode in \code{src/networks/lioness.py}) for regression-friendly scaling that does not pretend to variance-stabilize. Per-edge regression on a full $2\times 2\times 2$ factorial used limma empirical-Bayes-moderated cell-means parameterization \citep{ritchie2015limma} with explicit guard against double-adjustment for batch / deconvolution / surrogate variables already removed during upstream QR residualization.

\subsection{Network embeddings and rewiring statistics}

Predicted contrast networks were embedded via multi-seed \pkg{PecanPy} \pkg{node2vec} \citep{grover2016node2vec, liu2021pecanpy} ($d{=}128$, $p{=}0.25$, $q{=}4.0$, walk length 80, 200 walks per node, 10 random seeds) in \code{signed\_split} mode, where positive and negative edge weights are embedded into separate $d/2$-dimensional spaces and concatenated, preserving sign information that the previous \code{abs}-based mode discarded. Embeddings were aligned across seeds and contrasts by orthogonal Procrustes \citep{schonemann1966procrustes} on a pre-registered set of $\geq 50$ housekeeping anchor genes committed to \code{config/anchor\_genes.yaml} prior to any embedding computation. Per-gene rewiring was the row-wise cosine distance between aligned FLT and condition-control embeddings averaged across seeds. Gene-level inference used a sign-aware sum-of-$t$-statistic test, $T_g = \sum_{e\in N(g)} t_e / \sqrt{|N(g)|}$, with empirical $p$-values from $K{=}1{,}000$ label permutations rather than Stouffer's $Z$ on per-edge $p$-values (Stouffer was observed to saturate to numerical zero on partial-correlation skeletons under edge-dependence regimes typical of this cohort).
...
\subsection{Sample-specific LIONESS features predominantly fail to exceed expression baselines, with one stratum-specific exception}

The leakage-safe Phase 8 evaluation tested whether LIONESS-derived per-sample features improve FLT-versus-GC classification beyond an expression-only baseline. Under the corrected raw-contribution LIONESS with per-edge rank normalization and \code{signed\_split} embedding (run \code{run\_20260517\_213205\_2500g}), network features failed to exceed the expression baseline in three of four Age $\times$ Arm strata across all four LIONESS pool modes (effective $N \in \{5, 20, 40, 80\}$), but achieved \emph{parity} with the expression baseline in the LAR-Young stratum specifically when using sparse-edge feature engineering with $L_1$ logistic regression (network advantage $= 0.0$ at LAR-Young across all pool modes; Table~\ref{tab:phase8}). The result is biologically interpretable: LAR-Young is the only stratum in the cohort where both FLT and GC mice were sacrificed and dissected on Earth on matched timelines after 24-day post-flight recovery, eliminating the on-orbit-dissection confound that we discuss in Section 4.3.

\begin{table}[h]
\centering\small
\begin{tabular}{lcccc}
\toprule
Stratum & Network RF acc.\ & Best network feature acc.\ & Expression RF acc.\ & Best network advantage \\
\midrule
ISS-T Young & 0.5 & 0.5 (RF, node strength) & 0.9 & $-0.4$ \\
ISS-T Old & 0.2 & 0.2 (RF, node strength) & 0.9 & $-0.7$ \\
LAR Young & 0.6 & \textbf{0.6 ($L_1$-LR, sparse edges)} & 0.6 & $\mathbf{0.0}$ \\
LAR Old & 0.1 & 0.1 (RF, node strength) & 0.8 & $-0.7$ \\
POOLED ($n{=}40$) & 0.45 & 0.45--0.6 ($L_1$-LR, sparse edges) & 0.5 & $-0.05$ to $-0.125$ \\
\bottomrule
\end{tabular}
\caption{Phase 8 leakage-safe LOO-CV under corrected LIONESS (\code{raw\_ranknorm}, \code{signed\_split}). Network features tie the expression baseline in LAR-Young (network advantage $= 0.0$ with sparse-edge feature engineering and $L_1$-LR) and fall short in the three on-orbit-dissection-confounded strata. The result is invariant to pool mode; values shown are for \code{age\_arm\_envgroup} ($N{=}5$) and replicate at $N{=}20, 40, 80$ within $\pm 0.05$. Best classifier and feature type are listed; full pool-mode table in supplement.}
\label{tab:phase8}
\end{table}

Comparison against the previous-run (Fisher-$z$ LIONESS) Phase 8 result is informative. Under the previously-deployed \code{z\_contribution} transform, LAR-Young network advantage was $-0.5$ (uniform across pool modes); under the corrected \code{raw\_ranknorm} transform it is $0.0$ (also uniform across pool modes). The Fisher-$z$-on-LIONESS-contributions was therefore methodologically obscuring whatever per-sample signal was present in LAR-Young. ISS-T-Young also improved from $-0.9$ to $-0.4$ network advantage, though it remains negative. The two ISS-T strata remain firmly negative ($-0.4$ to $-0.7$), as does LAR-Old ($-0.7$). The corrected LIONESS does not rescue Phase 8 as a uniformly positive predictive-validation result, but it does identify LAR-Young as the single stratum where sample-specific network features carry classification-relevant information at this cohort size.
...
The principal finding of this study is methodological: at $n{=}5$ biological replicates per Age $\times$ Arm $\times$ Environment-Group cell, the detectable network-scale signal in mouse kidney spaceflight transcriptomics is coordinated module activity shift, not broad co-expression topology rewiring. This conclusion rests on the convergence of three methodologically independent analyses. Per-sample LIONESS edge weights, under a corrected raw-contribution-plus-rank-normalization transform that treats the LIONESS output as a per-sample contribution rather than a Fisher-z-transformable correlation, processed through Procrustes-aligned multi-seed \pkg{node2vec} embeddings with sign-preserving \code{signed\_split} edge embedding \citep{kuijjer2019estimating, grover2016node2vec, liu2021pecanpy}, do not exceed an expression-only baseline under leakage-safe leave-one-out cross-validation in three of four Age $\times$ Arm strata. The single exception is LAR-Young, where network features tie the expression baseline (network advantage $=0.0$ under \code{sparse\_edges} feature engineering with $L_1$ logistic regression), notably the only stratum in the cohort free from the on-orbit-dissection sample-collection confound. The result is invariant to the LIONESS sample pool from $N{=}5$ to $N{=}80$. Direct group-level differential correlation testing, which uses the same partial-correlation skeleton but compares FLT versus GC correlation matrices without the LIONESS step \citep{tesson2010diffcoex}, is null in every Age $\times$ Arm stratum with gene rankings uncorrelated with the LIONESS rankings. WGCNA module-trait association \citep{langfelder2008wgcna} identifies twelve modules with significant trait associations, with the lead module (grey60) showing the strongest Flight effect ($q < 10^{-4}$) and Flight $\times$ Arm interaction ($q < 10^{-4}$) of any module. Critically, GC-reference module preservation \citep{langfelder2011module} demonstrates that the grey60 gene set is strongly preserved in a flight-naive reference network ($Z_{\text{summary}} = 20.6$ for the 40/48 grey60 genes that map into a single GC-only module). Activity shift versus topology remodeling: the orchestra section gets louder, but the musicians are still seated together.

\subsubsection*{The LAR-Young exception: why a single stratum reaches parity}

That LAR-Young is the single stratum where corrected LIONESS features tie the expression baseline is itself biologically and methodologically informative. The four RRRM-2 Age $\times$ Arm strata differ along three coupled axes that affect what per-sample network features can be expected to recover. ISS-T strata involve on-orbit sacrifice and on-orbit dissection, introducing a documented sample-collection-protocol confound \citep{galazka2020rnaseq} that adds large-magnitude transcriptional variation orthogonal to flight-state biology. LAR strata involve Earth sacrifice and Earth dissection but at different post-flight recovery durations: LAR-Old mice are at the same recovery timepoint as LAR-Young (24 days post-flight) but differ in baseline aging burden. LAR-Young is therefore the only stratum where (a) FLT and GC samples were dissected on Earth on matched timelines, (b) the on-orbit-dissection confound is absent, and (c) the baseline-aging burden is minimized.

The observation that corrected LIONESS features achieve classification parity with the expression baseline specifically in LAR-Young, with no other stratum showing this parity, is consistent with the hypothesis that the on-orbit-dissection confound \emph{contaminates} per-sample classification in the ISS-T strata to a degree that overwhelms whatever true biological signal LIONESS captures, and that the baseline-aging burden in LAR-Old similarly degrades classification. Under this interpretation, LAR-Young is the cleanest stratum and per-sample network features perform exactly as well as expression in that clean stratum --- which is to say, they neither dominate nor underperform a simpler baseline. This is the expected behavior of a method that captures signal redundant with expression at small $N$, not the expected behavior of a method that captures signal orthogonal to expression. The qualitative conclusion (per-sample networks are redundant with expression at this cohort size) is preserved; the quantitative result (uniform negative network advantage) is no longer accurate, and the manuscript reports the stratum-dependent result honestly.
