...
\title{\textbf{A matrix-high / DCT-low kidney remodeling direction recurs across independent mouse spaceflight transcriptomes and aligns with the published Cosmic Kidney Disease NCC phenotype}\\[0.35em]
\large A cross-cohort transcriptomic reproducibility analysis of RRRM-2, OSD-513, and OSD-253, anchored to OSD-462 (RR-10) protein and phosphoprotein data}
...
\noindent\textbf{Purpose and scope of this draft.} This draft is framed deliberately as a \emph{cross-cohort transcriptomic reproducibility study}, not as a discovery paper. The definitive multi-omic, physiological, and morphological characterization of spaceflight-induced renal dysfunction --- including chronic dephosphorylation of the distal convoluted tubule Na--Cl cotransporter NCC --- has already been published as the Cosmic Kidney Disease study \citep{siew2024cosmic}. The contribution here is to ask, with an orthogonal cross-cohort design built from datasets that were \emph{not} part of that study, whether the same renal remodeling direction is independently detectable and reproducible at the transcriptome level, and to delineate honestly what transcriptomic pathway and network analysis can and cannot add.

\vspace{0.6em}
\small
\begin{tabularx}{\textwidth}{>{\bfseries}p{0.22\textwidth}X}
\toprule
Question & Does a matrix-high / DCT-low kidney remodeling direction reproduce across independent mouse spaceflight RNA-seq cohorts, and does it correspond to the published OSD-462 / RR-10 protein and phosphoprotein evidence of NCC transport-pathway suppression? \\
\midrule
...
\midrule
Main supported result & A matrix-high / DCT-low remodeling direction recurs across RRRM-2 ISS-T, OSD-513, and OSD-462 RNA. In OSD-462, this RNA direction does \emph{not} translate to transporter or matrix protein abundance, but the WNK--SPAK/OSR1--NCC activating phosphorylation cluster is specifically and strongly suppressed --- independently reproducing the Cosmic Kidney Disease NCC finding by an orthogonal route. LIONESS/node2vec network candidates show no independent protein/phosphoprotein support. \\
\midrule
Mechanism boundary & OSD-253 suggests TLR4/innate association, not TLR4 requirement. The RRRM-2 LAR arm shows attenuation, not a confirmed reversal; age-stratified effects are underpowered at $n=5$ per cell. \\
\bottomrule
\end{tabularx}
\normalsize
...
The matrix-high / DCT-low kidney flight direction is reproducible: it recurs across RRRM-2 ISS-T, OSD-513, and OSD-462 RNA. At the protein level in OSD-462 it is \emph{not} an abundance change; it corresponds instead to suppressed NCC/SPAK activating phosphorylation, concordant with the published Cosmic Kidney Disease phenotype \citep{siew2024cosmic}. Transcriptomic network methods (LIONESS, node2vec) did not translate to protein-level support and are reported as a negative methodological result. RRRM-2 cannot support the original age-rewiring claim; the LAR arm shows attenuation rather than reversal.
...
\noindent
Spaceflight harms the kidney. A recent landmark study, Cosmic Kidney Disease, showed using kidney tissue, urine, and blood from mice, rats, and astronauts that spaceflight chemically switches off a key salt-handling transporter in the distal kidney tubule, called NCC. This work does not try to re-discover that. Instead it asks a narrower, practical question: if several \emph{separate} mouse kidney spaceflight experiments are analyzed with gene-activity (RNA) data, do they all point in the same biological direction, and does that direction match the established NCC finding? We find that they do. Three independent mouse cohorts show the same RNA pattern --- more matrix/scarring-type activity and less distal-tubule transport activity. When we then check the one cohort that also has protein and protein-modification data, the amount of the transporter protein barely changes, but its activating chemical modification is strongly reduced --- the same NCC switch-off reported by the Cosmic Kidney study, recovered here by a completely different route. We also tested whether fashionable ``network'' analysis methods added anything; honestly, they did not. The value of this paper is reproducibility and careful method evaluation, not a new disease.

\vspace{0.8em}

\begin{abstract}
\noindent
The Cosmic Kidney Disease study established that spaceflight induces renal transporter dephosphorylation --- prominently of the thiazide-sensitive Na--Cl cotransporter NCC in the distal convoluted tubule (DCT) --- together with DCT remodeling, defining a primary spaceflight tubulopathy \citep{siew2024cosmic}. We do not re-investigate that phenotype. Instead we ask whether a corresponding renal remodeling \emph{direction} is independently reproducible across mouse spaceflight transcriptomes, and we evaluate what transcriptomic pathway and network analysis contributes beyond standard differential expression. We analyzed RRRM-2 (OSD-771), an age-by-arm-by-flight mouse kidney RNA-seq dataset, with the independent kidney flight cohorts OSD-513 and OSD-253, using residualized expression, curated pathway and mechanism scores, WGCNA modules, and LIONESS/node2vec network summaries combined into cross-cohort contrast vectors. We then built a pre-registered multi-omic anchor against OSD-462 (RR-10), which carries kidney RNA-seq, TMT proteomics, and phosphoproteomics.

RRRM-2 could not support the original within-cohort age-rewiring claim because control-aging directions were unstable at $n=5$ per cell. At the flight-vector level, the RRRM-2 ISS terminal-dissection arm aligned with OSD-513 (pathway cosine 0.641; 95\% CI 0.351 to 0.814) along a matrix-high / DCT-low tubulointerstitial direction; the live-return arm showed attenuation rather than a confirmed reversal. In the multi-omic anchor, the OSD-462 RNA flight effect reproduced the same matrix-high / DCT-low direction (pathway-vector cosine 0.87, 95\% CI 0.65--0.90; recurrence gate passed). Pre-registered protein-abundance concordance was \emph{null} for every targeted gene set, and matrix proteins moved opposite their transcripts; total NCC protein was unchanged ($+0.09$). However, the WNK--SPAK/OSR1--NCC activating phosphorylation cluster was specifically suppressed: NCC regulatory-cluster phosphosites (Thr53, Thr58, Thr65, Thr68; composite Ser65;Thr68 effect $-1.56$, $p=9.3\times10^{-7}$) and SPAK Ser383 ($-0.79$) were strongly down in flight, while non-regulatory NCC phosphosites were unchanged. This independently reproduces the Cosmic Kidney Disease NCC dephosphorylation finding through an orthogonal cross-cohort transcriptome-anchored route, and shows the transcriptomic signal corresponds to transport-pathway \emph{activity} rather than transporter or matrix \emph{abundance}. LIONESS/node2vec network candidates showed no enrichment for protein- or phosphoprotein-changing genes ($p\geq0.23$): a clean negative result on the added value of network methods. The contribution of this work is transcriptomic reproducibility of a clinically important renal spaceflight phenotype, an explicit transcript--protein--phosphoprotein dissociation, and an honest delineation of the limits of network-based inference.
\end{abstract}

\vspace{0.3em}
\noindent\textbf{Keywords:} spaceflight transcriptomics; mouse kidney; reproducibility; OSD-771; RRRM-2; OSD-513; OSD-462; RR-10; cosmic kidney disease; multi-omics; proteomics; phosphoproteomics; distal convoluted tubule; NCC; SLC12A3; WNK-SPAK/OSR1; extracellular matrix; LIONESS; node2vec.
...
This study is a cross-cohort transcriptomic reproducibility analysis of the spaceflight kidney remodeling response. A matrix-high / DCT-low remodeling direction recurs across three independent mouse kidney spaceflight cohorts --- RRRM-2 ISS-T, OSD-513, and OSD-462 RNA --- establishing it as a reproducible transcriptomic feature rather than a single-study artifact. Anchored against OSD-462 (RR-10) protein and phosphoprotein data under a pre-registered plan, the transcriptomic direction does not correspond to transporter or matrix protein abundance but to specific suppression of the WNK--SPAK/OSR1--NCC activating phosphorylation cluster, independently reproducing the NCC dephosphorylation phenotype of the Cosmic Kidney Disease study \citep{siew2024cosmic} through an orthogonal route. The contribution is therefore reproducibility, an explicit transcript--protein--phosphoprotein dissociation showing the signal is one of transport-pathway activity rather than abundance, and an honest negative result: co-expression network and embedding methods nominated candidates that did not translate to protein-level biology. RRRM-2 cannot support the original age-rewiring claim, and the live-return arm shows attenuation rather than reversal. The defensible value of the work is that it confirms a clinically relevant renal spaceflight phenotype across cohorts and analytic routes, and delineates clearly what transcriptomic and network analysis can and cannot establish on their own.
...
\end{center}
\vspace{0.3em}
\noindent\textit{Draft v8 prepared May 2026. This draft is framed as a cross-cohort transcriptomic reproducibility study; the spaceflight NCC dephosphorylation phenotype it confirms was first established by the Cosmic Kidney Disease study \citep{siew2024cosmic}. Reference metadata should be curated against a journal style file before submission.}
