...
def main():
    ...
    print(f"      [OK] Loaded {rsem.shape[0]} genes x {rsem.shape[1]} samples")
    
    # 2. Read metadata
    print(f"\n[2/5] Reading ISA metadata...")
    print(f"      Path: {META_PATH}")
    meta = read_metadata_txt(META_PATH)
    print(f"      [OK] Loaded metadata for {len(meta)} samples with {len(meta.columns)} columns")
    ...
    print(f"      [OK] Metadata aligned with {len(parsed.columns)} additional design columns")
    ...
    print(f"        [OK] {star_aligned.shape[0]} genes x {star_aligned.shape[1]} samples")
    ...
    print(f"        [OK] {vst_aligned.shape[0]} genes x {vst_aligned.shape[1]} samples")
    ...
    for name, bundle in bundles.items():
        ...
        print(f"  [OK] {name:20s}: {counts_out}")
