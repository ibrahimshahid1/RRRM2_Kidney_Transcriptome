...
def build_design_matrix(meta: pd.DataFrame, clr: pd.DataFrame,
    ...
    return X.astype(np.float64), dct_col_idx
...
def main():
    ...
    X, dct_col = build_design_matrix(meta, clr, cell_cols, tech_cols, args.drop_segment)
    ...
    print(f"\n8) Computing DCT specificity (|β_DCT| dominant among segments)...")
    ...
    results["dct_dominant"] = is_specific
    sig_specific = sig_pos & is_specific
    print(f"   Of {sig_pos.sum()} significant, {sig_specific.sum()} are DCT-dominant")
    ...
    cols_order = ["gene", "beta_dct", "t_stat", "p_value", "q_BH",
                  "partial_r2", "dct_dominant", "bootstrap_freq"]
    ...
    print(f"  + DCT-dominant:           {sig_specific.sum()}")
